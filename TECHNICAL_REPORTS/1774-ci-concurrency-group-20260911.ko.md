...
**확인하지 못했고, 일부러 주장하지도 않는 것.** 새 푸시가 진행 중 실행을 실제로 대체하는지는 살아 있는 GitHub 인프라에 두 푸시를 경쟁시켜야만 드러나는 브랜치 간 동작이고, 이 PR의 CI가 초록이라는 사실은 그것을 보여 주지 않는다. 연출된 이중 푸시는 비용 때문에 접었다. `.github/workflows/ci.yml`은 `changes`의 경로 필터 넷(`rust`, `mlx_pin`, `xla_link`, `cuda_arch`) 모두에 들어 있어서, 이 브랜치에 푸시할 때마다 GB10 잡 넷이 전부 시작되고 그중 둘은 `timeout-minutes: 120`을 달고 있다. 부하를 덜어 준다는 주장을 증명하겠다고 단 하나뿐인 공용 러너에 가능한 가장 무거운 부하를 만들어 내는 것은 잘못된 거래다. 이슈가 제시한 검증 절차는 이 필터 적용 범위를 셈에 넣지 않았다.

연출 없이도 동작은 관찰된다. 이 PR 브랜치는 이 리포트를 실어 나르는 두 번째 푸시를 어차피 해야 할 작업으로 받았기 때문이다. 그 결과로 일어난 대체, 또는 일어나지 않음은 `gh run list --workflow ci.yml --branch chore/issue-1774-ci-concurrency`에서 보이며, 여기서 미리 단정하지 않고 PR에 보고한다.
