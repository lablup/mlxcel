...
- **테스트 커버리지**: 16개 추가. 8개는 합성 메시지로 protobuf 재작성을 검증하고, 2개는 실제 `tokenizer.model`을 읽으며(체크포인트가 없으면 공용 pinned 게이트로 skip), 6개는 두 거부와 두 비활성 키 사례를 포함한 detection을, 1개는 `add_prefix_space` 파싱을, 1개는 `mlxcel arch`에 계열이 나타나는지를 확인한다.
...
| 테스트 | 3개 파일 | 16개. 그중 2개는 pinned 게이트 뒤에서 실제 체크포인트를 읽는다 |
