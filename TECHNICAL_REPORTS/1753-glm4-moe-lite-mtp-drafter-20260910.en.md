...
- The `MAX_BLOCK_SIZE` refusal message carries two runs of 14 literal spaces: a wrapped string literal lost its `\` continuations and rustfmt joined the line. The test asserts only that the message contains `20000` and `maximum`, so it does not catch the formatting.
- `--force` removes the stale index but not stale `model-XXXXX-of-YYYYY.safetensors` shards. With the index gone, `collect_shard_paths` falls through to a glob and would read the drafter's `model.safetensors` alongside the orphans. Refusing a forced overwrite of a sharded directory outright is the cheaper fix than deleting shards on the user's behalf.
- The `--q-bits` help text says `(2, 3, 4, 5, 6 or 8)` but the only validation is `validate_quantization_params`, which accepts `1..=32`. A depth MLX's affine packing does not support passes the tool and fails at load.
