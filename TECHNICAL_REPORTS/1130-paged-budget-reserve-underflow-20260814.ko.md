...
- **`Some(0)` 계약은 문서에만 있고 덮여 있지 않았다.** `resolve_paged_block_budget`은 한 블록 아래로 내려간 예산이 `Some(0)`이 된다고 적어 두었고, 예약분보다 작은 예산이 대략 `2^47` 블록이 아니라 거기로 떨어지게 만드는 것이 `saturating_sub`다. 그런데 예약분보다 작은 예산을 요청하는 테스트가 하나도 없었으므로 그 포화 연산은 곁다리로만 덮여 있었다.
...
| 나중에 누가 구현의 `saturating_sub`를 `-`로 되돌려 작은 예산에 대략 `2^47` 블록을 발급한다 | High | Low |
...
비테스트 코드에서 예약분을 예산에서 빼는 지점은 정확히 하나이고 이미 포화 연산이었다. 다만 결론을 grep에만 기대게 두지는 않았다. 새로 넣은 `resolve_block_budget_below_the_workspace_reserve_is_zero_blocks`는 `0`, `1`, `workspace / 2`, `workspace - 1`, `workspace` 예산으로 실제 함수를 호출해 각각 `Some(0)`을 요구한다. 뺄셈이 감싸 돌았다면 뒤의 셋은 대략 `2^47` 블록이 된다. `u64::MAX`에 가까운 예산을 128 KiB 블록으로 나눈 값이지 64비트 호스트에서 `usize::MAX`가 되지는 않는다. 이 구분을 정확히 적어 두는 이유는, 이 줄을 지키는 주석의 목적이 나중에 누가 `-`로 "단순화"하는 것을 막는 데 있기 때문이다. 이 테스트가 통과하므로 구현이 깨끗하다는 것은 눈으로 읽은 결론이 아니라 측정된 결론이다.
...
### 3.4 맞는 문장 하나로 때우지 않고 지시자별로 경고를 나눈다

처음에는 두 지시자에 같은 메시지를 주었다. `Auto`에도 맞는 문장이라는 근거였다. 예약분에 블록 하나를 더한 값에 못 미치는 예산이라는 것은 두 경로가 함께 부딪힌 상황 그대로다. 리뷰가 이를 반려했고 그 판단이 옳다. 진단 문구가 넘어야 할 기준은 참이냐가 아니다. `Auto`는 두 바이너리의 기본값이고, 모델이 KV에 쓸 자리를 아예 남기지 않을 때 0 블록에 도달한다. 실제 부족분이 수십 기가바이트인데 16 MiB 예약분을 지목하면 운영자를 `MLXCEL_PAGED_DECODE_V2_TARGET_CTAS`로 보내게 된다. 아예 다른 손잡이다.

이제 이 분기는 지시자로 갈린다. `Auto`는 원래 문구를 그대로 쓴다. 애초에 그 경우에 맞는 진단이었다. `Bytes`는 요청한 예산, 예약분, 블록 하나를 발급하는 데 필요한 최소 예산을 함께 보고하며, 셋 다 이 모듈의 `format_bytes`를 거쳐 원시 바이트 수가 아니라 크기로 읽힌다. `PagedBudgetDirective`는 `Copy`이고 지시자가 아직 스코프에 있으므로 분기 비용은 두 줄이다. 어느 경로에서도 해석 로직은 바뀌지 않았다.
...
- `resolve_worker_paged_block_budget`: 0블록 분기가 두 지시자에 한 문장을 쓰지 않고 지시자별로 갈린다. `Auto`는 원래 문구를 유지하고, `Bytes`는 요청한 예산, 워크스페이스 예약분, 블록 하나를 발급하는 최소 예산을 모두 `format_bytes`를 거쳐 보고한다.
