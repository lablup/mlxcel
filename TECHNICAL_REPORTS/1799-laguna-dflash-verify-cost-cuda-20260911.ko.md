...
게이트는 2에서 32행의 query와 선행 컨텍스트를 가진 모든 CUDA 마스크 SDPA 호출에 닿는다. Ampere 이후에서 head_dim 128 이하인 모든 패밀리의 투기적 검증 shape(Laguna, 그리고 타깃의 head_dim이 128인 모든 MTP나 DFlash 페어링)과 청크 프리필의 마지막 짧은 청크다. 그 호출들은 이제 cuDNN 대신 MLX fallback의 산술로 돌므로, cuDNN을 통해 클래식 체인과 바이트 동일했던 페어링은 원칙적으로 동점에서 뒤집힐 수 있다. Laguna 프로브는 다른 이유(logit 동점에서 `qmm_sm80`과 `fp_qmv`의 반올림 차이, 여기서는 범위 밖)로 이미 이 호스트를 거부하며 게이트는 건드리지 않았다. Qwen 3.5(head_dim 256)는 애초에 cuDNN에 들어가지 않으므로 구조적으로 영향이 없고, 그 투기적 이득은 새 바이너리에서 다시 측정했다(위 표). Metal과 CPU는 영향이 없다. 캐시 크기 기본값은 모든 CUDA 프로세스에 닿지만 서로 다른 SDPA shape이 쌓일 때만 메모리를 쓴다.

남는 것은 행당 expert 항(행당 2.5 ms. 같은 expert로 라우팅되는 행들이 읽기를 공유하지 않는 한 MoE 검증에 내재한다)과 1행 gemv 대비 bf16 GEMM의 고정 12 ms다. 수정 후 폭 곡선은 블록 4에서 정점이고 체크포인트 기본값 16은 여전히 가장 나쁜 폭이며, 이는 #1797의 정책 질문이다. GB10의 기본 그래프 예산이 이 MoE 모델에서 캡처를 순손실로 만든다는 클래식 arm 발견(그래프 끄면 +12%, 두 예산 다 올리면 +16%, Qwen 3.5와 반대 부호)은 #1798을 위해 기록했고 여기서는 바꾸지 않았다.
