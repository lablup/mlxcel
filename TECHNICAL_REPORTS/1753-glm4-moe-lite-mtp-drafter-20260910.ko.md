...
- #1763에서 해결: `--force`는 이제 출력 디렉터리에 드래프터 자신의 `model.safetensors`가 아닌 `*.safetensors`(이전 체크포인트의 `model-*.safetensors` 샤드나 `consolidated.safetensors`)가 하나라도 있으면 삭제 대신 거부하며, 그 파일을 이름으로 지목하고 모든 파일을 그대로 둔다. 그런 파일이 없을 때는 여전히 낡은 인덱스를 지운다.
- #1763에서 해결: `split-mtp`는 이제 어떤 텐서 작업이나 디렉터리 변경도 하기 전에 `--q-bits`를 `SUPPORTED_AFFINE_BITS`(`{2, 3, 4, 5, 6, 8}`)로, `--q-group-size`를 `SUPPORTED_AFFINE_GROUP_SIZES`(`{32, 64, 128}`)로 검증하여 도움말과 강제되는 집합이 일치한다. 공유 로드 시점 검증인 `validate_quantization_params`의 범위 검사(`1..=32`)는 그대로다.
