...
- `MAX_BLOCK_SIZE` 거부 메시지에 14칸짜리 공백 덩어리가 두 개 들어 있다. 줄바꿈된 문자열 리터럴이 `\` 연속 표시를 잃었고 rustfmt가 줄을 합쳤다. 테스트는 메시지가 `20000`과 `maximum`을 포함하는지만 단언하므로 이 서식은 잡히지 않는다.
- `--force`는 낡은 인덱스를 지우지만 낡은 `model-XXXXX-of-YYYYY.safetensors` 샤드는 지우지 않는다. 인덱스가 사라지면 `collect_shard_paths`가 glob으로 떨어져 drafter의 `model.safetensors`와 고아 샤드를 함께 읽는다. 사용자 대신 샤드를 지우는 것보다 샤드로 나뉜 디렉터리에 대한 강제 덮어쓰기를 아예 거부하는 편이 싸다.
- `--q-bits` 도움말은 `(2, 3, 4, 5, 6 or 8)`이라고 적혀 있지만 실제 검증은 `1..=32`를 받는 `validate_quantization_params` 하나다. MLX의 affine 패킹이 지원하지 않는 깊이가 도구를 통과해 로드에서 실패한다.
