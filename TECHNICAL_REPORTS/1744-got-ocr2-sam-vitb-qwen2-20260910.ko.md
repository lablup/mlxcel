...
- **4비트 변환본은 greedy 일치 게이트에 넣지 않았다.** CLI에서 페이지를 정확히 옮겨 적고 양자화 블록도 `got_text_config`로 단위 테스트되지만, 레퍼런스 오라클은 원본 fp32 가중치로 돌므로 양자화된 greedy 일치는 의미 있는 비교가 아니고 주장하지 않았다. 8비트 변환본은 내려받지 않았고 전혀 돌리지 않았다.
