...
Control plane도 정확한 cardinality와 순서를 지켜야 한다. Prompt placeholder 하나는 유효 audio frame마다 token 하나로 확장해야 한다. Right-padding row는 tower에 들어가면 안 된다. Image/audio 혼합 요청에서는 공개 모델과 같은 순서로 feature를 scatter해야 한다. 서버 prompt materialization은 flatten된 audio를 마지막 user turn에 다시 묶지 않고 모든 과거 및 현재 media part를 원래 위치에 유지해야 한다. Prepared multimodal input은 text-only speculative 경로에 들어가면 안 된다.
...
| Compact bridge | Clip별 유효 row만 요청 순서대로 계산하고 연결하며 실제 FFT를 6,000회로 제한하고 frame마다 cancellation 검사 |
...
| Prompt 통합 | 서버 ordinal sentinel을 원래 message/part 위치에서 완전한 image/audio 구조로 materialize하고 raw reserved media token을 거부하며 명시적 CLI 합성 유지 |
...
3. Inkling feature extractor는 clip마다 STFT left context를 독립적으로 더하고 유효한 50 ms frame에 대해서만 80개 log-mel 값을 계산한다. 각 FFT 전에 cancellation을 검사하고 실제 transform 수가 6,000을 넘으면 추출 전에 거부한다.
4. Quantizer는 값을 `[-7, 2]`로 clamp하고 아래 반올림한 15개 boundary에 대한 strict 비교 횟수를 세어 `[0, 15]` 범위의 int32 ID를 만든다.
5. Host bridge는 이미 compact한 `[frame, 80]` row를 clip 순서로 양자화한다. Prompt preparation은 clip마다 audio placeholder 하나를 해당 valid-frame 수만큼 정확히 확장한다.
...
### 4.2 Tower 전에 padded transform을 피하기

모델 계약에는 padding mask가 없고 모든 clip을 가장 긴 clip 길이까지 transform하면 FFT 작업량이 16-clip 요청 제한만큼 늘어날 수 있다. Production extractor는 각 clip의 valid frame 수를 직접 순회하고 매 transform 전에 cancellation을 검사하며 FFT 실행을 구동하는 동일한 counter에 6,000-frame 요청 상한을 적용한다. 정확한 allocation cardinality를 검증한 뒤 `[total_valid_frames, 80]` MLX array를 만든다. Scatter 전에 placeholder 수와 tower output row 수가 정확히 같아야 한다.
...
### 4.4 서버 media의 message 및 part 순서를 보존하기

Request renderer는 transport content를 chat template용으로 flatten하면서 증가하는 private image/audio sentinel을 부여한다. Inkling worker는 rendering 후 sentinel을 parse하고 누락되거나 순서가 어긋난 marker를 거부한 뒤 각 원래 part를 제자리에서 복원한다. Image part는 완전한 `message_user/content_image/image/end_message` block이 되고 audio part는 valid-frame 확장 전에 완전한 `message_user/audio_bos/audio/audio_end/end_message` block이 된다. Media 전후 text는 과거 turn을 포함해 별도 text part에 남으며 image 또는 audio placeholder로 직접 encode되는 text는 신뢰할 수 없는 reserved-token 입력으로 거부한다. 회귀 테스트는 system context, 과거 및 현재 audio, image/audio 혼합 순서를 포함한다.
...
독립 리뷰에서 sentinel 제거 후 발생하는 cross-turn media rebinding과 admitted valid-frame 수보다 약 16배 많은 FFT를 수행할 수 있는 padded batch extraction이라는 HIGH 두 건을 발견했고 최종 구현에서 해결했다. 구현 handoff 시점에 남은 CRITICAL 또는 HIGH finding은 없다. 최종 구현은 다음을 보강했다.
...
- Ordered server media를 원래 message/part boundary에서 복원하고 raw reserved image/audio token을 거부한다.
- Production dMel extraction은 admission을 통과한 valid-row FFT만 수행하고 frame 단위 cancellation을 반영한다.
...
| 게이트 | 결과 |
| --- | --- |
| `cargo test -p mlxcel --lib inkling -- --nocapture` | 통과, 83/83 |
| `cargo test -p mlxcel --lib inkling_ordered_prompt -- --nocapture` | 통과, 2/2 |
| `cargo test -p mlxcel --lib compact_batch_ -- --nocapture` | 통과, 3/3 |
