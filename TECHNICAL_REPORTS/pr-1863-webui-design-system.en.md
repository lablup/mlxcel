...
The [canonical hosted WebUI job at `98fe9df2`](https://github.com/lablup/mlxcel/actions/runs/34802205815/job/103846995858) passed typecheck, lint, all 76 unit tests, all 20 browser cases and deterministic bundle verification with digest `858f5775f548532b3fa93942c7f063e06b83f427d107b47c82678f3564638f7f`. The actual model-free production server built from `62c4f259` separately passed both supplied-server CSP cases (1440 light and 390 dark), including real response policy, no policy violations or external requests, Select geometry/Escape focus, measured progress, reduced motion, axe and browser AX heading semantics. Static hosted tests alone were not used to claim that result. Production sources/assets are identical between these two heads.

The user explicitly deferred the migrated-DOM Safari/VoiceOver/native-zoom checks to final integrated acceptance because the current remote environment cannot support those manual checks. Their status is DEFERRED, not PASS, and they are no longer a per-unit merge blocker. Run the shared-control and page-level native checks together after the whole implementation; historical 4185 acceptance is not reused for the changed DOM. Final integration after the download backend merge remains required. Status stays review, and no unavailable GB10 result is reported as passed. This decision changes documentation only; the immutable 4186 preview, production sources and bundle remain unchanged.

### Final UI-first integration decision

The central merge sequence is refined to merged #1838 followed by independently validated #1843; #1843 has no dependency on #1841. The later download-backend integration must preserve this UI and extend the current 44 contract fixtures to its 46-fixture coverage. Native Safari/VoiceOver/zoom acceptance remains DEFERRED to the final integrated implementation, not PASS and not a per-unit merge blocker. Consumers use the stable [shared export/exception matrix](https://github.com/lablup/mlxcel/blob/main/docs/webui/ui-common.md).

At `a01724df`, hosted run 34804568515 passed all executable non-GB10 jobs, including 76 unit tests, 20 browser cases and deterministic bundle verification. All four remaining queued jobs request exactly the `GB10` runner label with no assigned runner: cargo-clippy, OpenXLA feature compile, OpenXLA feature link and CUDA sm_70 compile. Only those unavailable-runner jobs are covered by the user's waiver. This final sequencing update changes documentation only; production source/assets remain identical to the actual two-case CSP validation and the hosted browser pass.
