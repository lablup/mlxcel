...
`pass2_matches_only_the_correct_reference`는 `TransformerBlock::forward_pass2`를 직접 호출해 그 레이어 출력을 테스트 안에서 조립한 레퍼런스와 비교하며 변종 다섯을 둔다. 올바른 것, `GateFromPreRope`, `GlobalReadsPass2Kv`, `LocalUnwindowed`, `BranchesSwapped`이다. 윈도우 4에 12토큰 실행에서 올바른 쪽과는 최대 절대 차 1e-5 미만을, 틀린 쪽 각각과는 1e-3 초과를 요구한다.

처음부터 진짜 함수를 부른 것은 아니다. 첫 판은 `forward_pass2`의 본문을 테스트 안에 그대로 옮겨 적어 "구현" 쪽으로 삼았다. 결국 코드 사본을 같은 사본의 변종과 비교한 셈이고, 저장소 어디에도 `forward_pass2`를 부르는 테스트가 없었다. 이 사실은 변이 테스트로 드러났다. 이 절이 지금 형태로 남아 있는 이유이기도 하다. 부르지도 않는 함수의 이름을 단 테스트는 겉보기에 멀쩡히 도는 테스트와 똑같아 보인다. 고친 방법은 그 함수를 실제로 부르고, 아무도 덮지 않던 다섯째 변종을 더한 것이다.

| `forward_pass2`에 넣은 변이 | 본문을 옮겨 적은 테스트 | 함수를 부르는 테스트 |
| --- | --- | --- |
| 전역 갈래와 지역 갈래를 맞바꿈 | 17개 통과 | 1개 실패, 최대 절대 차 0.147 |
| 게이트를 RoPE 이전 쿼리로 계산 | 17개 통과 | 1개 실패 |
| 전역 갈래가 2차 패스 K/V를 읽음 | 2개 실패 | 2개 실패 |
| 지역 갈래가 윈도우 없이 어텐션 | 2개 실패 | 2개 실패 |

조용히 틀린 출력을 내는 두 버그가 함수를 부르지 않는 판에서는 보이지 않았다. 둘 다 이미 덮었다고 적어 둔 것들이었다. 변이를 넣지 않으면 17개가 모두 통과한다.
