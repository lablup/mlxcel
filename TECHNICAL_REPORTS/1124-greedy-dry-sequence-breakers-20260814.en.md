...
- **The omission does not disable DRY, it strengthens it.** `dry_sequence_breakers` is not a knob that turns something on. It is the termination condition of the backward match in `apply_dry_penalty` (`src/lib/mlxcel-core/src/sampling.rs:846`, `if config.dry_sequence_breakers.contains(&window[p1]) { break; }`). An empty vector means the loop never breaks at a boundary token, so `match_len` can grow past the window the caller intended and `dry_multiplier * dry_base.powi(match_len - dry_allowed_length)` grows exponentially in that extra length. The breaker test precedes the equality test in the loop, so removing breakers can only lengthen the match or leave it unchanged: the penalty is never smaller than requested, and it is strictly larger whenever the tokens beyond the boundary happen to match as well. The failure mode is a penalty at or above what was configured, not a feature that quietly does nothing.
