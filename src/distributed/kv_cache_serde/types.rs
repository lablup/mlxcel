...
pub struct SerializableCacheState {
    ...
    /// Tokens already generated by the prefill node before handoff (#126 B2b).
    ///
    /// At handoff time this holds the prefill node's first sampled token (the
    /// token produced from the last prompt position). The decode node seeds its
    /// continuation from these so the very first decode step feeds the correct
    /// token and the emitted stream is identical to a single-node run. Empty for
    /// a pre-decode / metadata-only payload; rides inside the metadata JSON so it
    /// is bounded by the same `max_frame_bytes` frame cap as `token_history`.
    #[serde(default)]
