...
pub struct DeepSeekOcrPreprocessed {
    ...
    /// Per image: `(width_crop_num, height_crop_num)`. `(1, 1)` means no tiles.
...
impl DeepSeekOcrProcessor {
    /// Placeholder-token count for a `(w_crop, h_crop)` decision.
    /// Global mosaic `(nb+1)*nb` + view separator `1`, plus, when tiled,
    /// `(nt*w + 1) * (nt*h)`.
    pub fn placeholder_count(&self, w_crop: i32, h_crop: i32) -> i32 {
    ...
    pub fn preprocess(&self, images: &[image::DynamicImage]) -> DeepSeekOcrPreprocessed {
        ...
        for image in images {
            ...
            let (w, h) = (rgb.width(), rgb.height());
            // Crop only when larger than a single tile.
            let (cols, rows) = if w as i32 <= self.tile_size && h as i32 <= self.tile_size {
                (1, 1)
            } else {
                self.closest_grid(w, h)
            };
            ...
            counts.push(self.placeholder_count(cols, rows));
        }
...
mod tests {
    ...
    fn placeholder_counts_match_reference_formula() {
        ...
        assert_eq!(p.placeholder_count(1, 1), 273);
        // 800x600 picks grid (3, 2): 273 + (10*3+1)*(10*2) = 273 + 31*20 = 893.
        assert_eq!(p.placeholder_count(3, 2), 893);
    }
