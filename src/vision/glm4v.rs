...
//!
//! GLM-4V ViT + GLM-4 text backbone with sectioned even/odd MRoPE. Like the
//! Qwen-VL wrappers, the vision tower runs with packed sequences (`cu_seqlens`)
//! and its patch merger projects directly into the text hidden size; the text
//! model consumes 3D MRoPE position IDs computed by `compute_rope_index`.
...
/// GLM-4V VLM: GLM-4V ViT + GLM-4 text model with MRoPE.
pub struct Glm4vModel {
...
impl LanguageModel for Glm4vModel {
    fn forward(
        &self,
        input_ids: &MlxArray,
        caches: &mut [KVCache],
