...
//! GLM-4V MoE Vision-Language Model
//!
//! Reuses the GLM-4V ViT vision encoder ([`super::encoders::glm4v`]) and the
//! shared Qwen-VL image processor / prompt plumbing, paired with the GLM-4V MoE
//! text backbone. Vision features are merged LLaVA-style and the text model
//! consumes 3D MRoPE position IDs from `compute_rope_index`.
...
/// GLM-4V MoE VLM: GLM-4V ViT + GLM-4 MoE text model with half-split MRoPE.
pub struct Glm4vMoeModel {
    pub text_model: crate::models::glm4v_moe::Glm4vMoeTextModel,
    pub vision_encoder: encoders::glm4v::Glm4vVisionEncoder,
...
impl LanguageModel for Glm4vMoeModel {
    fn forward(
        &self,
        input_ids: &MlxArray,
        caches: &mut [KVCache],
