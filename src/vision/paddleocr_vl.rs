...
//!
//! Composes the NaViT dynamic-resolution vision encoder, the spatial-merge
//! projector, the OCR image processor, and the ERNIE-4.5 MRoPE text decoder.
//! Architecturally it follows the Qwen2-VL VLM contract: packed vision
//! sequences with `grid_thw`, LLaVA-style image-token replacement, and 3D
//! `[T, H, W]` MRoPE position ids computed once at prefill.
//!
//! Reference: mlx-vlm `paddleocr_vl/paddleocr_vl.py`.

use super::{connectors, encoders, merge, processors};
use crate::LanguageModel;
use crate::models::PaddleOcrTextModel;
...
pub struct PaddleOcrVlModel {
    pub text_model: PaddleOcrTextModel,
    pub vision_encoder: encoders::paddleocr_vl::PaddleOcrVisionEncoder,
    pub connector: connectors::paddleocr_vl::PaddleOcrProjector,
    pub processor: processors::paddleocr_vl::PaddleOcrVlProcessor,
...
impl PaddleOcrVlModel {
    /// Encode images, merge features at image-token positions, and store the
    /// 3D MRoPE position ids on the text model (fallback slot).
    ...
    /// (standard Qwen2-VL / PaddleOCR `get_rope_index`). Returns `[3, 1, seq]`.
...
impl LanguageModel for PaddleOcrVlModel {
    fn forward(
        &self,
        input_ids: &MlxArray,
        caches: &mut [KVCache],
