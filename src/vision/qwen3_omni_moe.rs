...
//! Qwen3-Omni MoE Vision-Language Model (thinker, stage 1).
//!
//! Composes the Qwen3-VL vision encoder (DeepStack) and the Qwen3-VL-MoE text
//! backbone, both reused unchanged, with the new Qwen3-Omni audio tower.
//! Audio feature rows scatter into the token stream at `audio_token_id`
...
use crate::audio::qwen3_omni_moe::Qwen3OmniAudioEncoder;
...
impl Qwen3OmniMoeModel {
    /// Image-only embeddings (the shared Qwen VL runtime path).
    pub fn get_input_embeddings(
        ...
        self.input_embeddings_multimodal(input_ids, Some((pixel_values, grid_thw)), None)
    }

    /// Embeddings with any combination of image and audio inputs. `audio` is
    /// the encoder output rows `(total_audio_frames, hidden)`, produced by
    /// [`Qwen3OmniAudioEncoder::forward`]; the row count must equal the number
    /// of `audio_token_id` placeholders in `input_ids`.
    pub fn input_embeddings_multimodal(
        &self,
        input_ids: &MlxArray,
        images: Option<(&MlxArray, &[(i32, i32, i32)])>,
        ...
        let mut grid_for_rope: &[(i32, i32, i32)] = &[];
        if let Some((pixel_values, grid_thw)) = images {
            grid_for_rope = grid_thw;
            ...
            // DeepStack: per-layer feature injection at visual positions.
        ...
        // MRoPE: image/video blocks get 3D grids; every other token (audio
        // placeholders included) gets sequential text positions.
        let position_ids = self.compute_rope_index(input_ids, grid_for_rope);
    ...
    /// Encode one audio clip (whisper-mel layout, `[num_frames][num_mel_bins]`
    /// row-major) into `(audio_out_len(L), hidden)` feature rows.
    pub fn extract_audio_features(&self, mel: &[f32], num_frames: usize) -> UniquePtr<MlxArray> {
        self.audio_encoder.forward(mel, num_frames)
    }

    /// Boolean mask of image/video token positions, `[batch, seq_len]`.
    ...
    /// 3D `[T, H, W]` position ids for mixed text+image sequences (same
    /// algorithm as `vision::qwen3_vl_moe`; audio tokens fall through the text
...
impl LanguageModel for Qwen3OmniMoeModel {
    fn forward(
        &self,
        input_ids: &MlxArray,
        caches: &mut [KVCache],
