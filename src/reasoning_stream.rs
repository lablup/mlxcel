...
mod tests {
    ...
    fn render_full_passthrough_is_byte_identical_when_no_channel() {
        ...
        assert_eq!(render_full(&gemma_markers(), text, false, true), text);
        // A non-thinking model is also byte-identical.
        assert_eq!(
            render_full(&ThinkingMarkers::default(), text, true, true),
            text
        );
    }
    ...
    fn render_full_hides_channel_by_default() {
        ...
        assert_eq!(render_full(&gemma_markers(), text, false, true), "answer");
        assert_eq!(render_full(&gemma_markers(), text, false, false), "answer");
    }
}
