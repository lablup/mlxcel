...
mod cache_isolation {
    ...
    /// Repro for the #674 parity battery: a cache-level chunked prefill
    /// (consecutive multi-token forwards continuing from the model-owned
    /// caches) must produce the same final-position logits as one
    /// single-pass prefill, for both layer families, including chunks that
    /// EXCEED the sliding window (the real-model shape is chunk 2048 over
    /// window 1024; `window` = 4 with `chunk` = 8 reproduces that ratio).
    #[test]
    fn gemma4_chunked_prefill_matches_single_pass() {
        for (layer_type, window) in [
            ...
            for piece in prompt.chunks(8) {
                let input = mlxcel_core::from_slice_i32(piece, &[1, piece.len() as i32]);
                let logits =
                    chunked.forward_last_logits(&input, &mut caches, None, piece.len() - 1);
            ...
            assert!(
                ...
                "{layer_type} window={window}: chunked prefill produced non-finite logits: {c_vals:?}"
            );
            let max_diff = s_vals
                .iter()
                .zip(&c_vals)
            ...
            assert!(
                max_diff < 1e-3,
                "{layer_type} window={window}: chunked prefill diverged from single-pass \
                 (max diff {max_diff}); single={s_vals:?} chunked={c_vals:?}"
            );
        }
    }
