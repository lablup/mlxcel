...
//! PaddleOCR-VL language model (ERNIE-4.5 decoder + MRoPE).
//!
//! PaddleOCR-VL pairs a NaViT dynamic-resolution vision encoder with a
//! lightweight ERNIE-4.5 text decoder. The decoder is architecturally the
//! ERNIE-4.5 dense stack (RMSNorm pre-norm, SwiGLU MLP, GQA attention with an
//! optional `head_dim` override and optional projection bias), but its position
//! encoding is Multi-dimensional RoPE (MRoPE) driven by 3D `[T, H, W]` position
//! ids so vision tokens carry spatial structure, exactly like Qwen2-VL.
//!
//! Reuses [`crate::models::qwen_mrope_state::MRopeState`] for per-sequence
//! MRoPE delta tracking so batched server requests do not leak positions.
//!
//! Reference: mlx-vlm `paddleocr_vl/language.py` (`PaddleOCRModel`,
//! `apply_multimodal_rotary_pos_emb` with `style="sectioned_half_split"`).
...
pub struct PaddleOcrTextConfig {
...
// MRoPE (sectioned half-split), mirrors the proven Qwen2-VL implementation.
struct MRoPE {
    inv_freq: Vec<f32>,
...
impl MRoPE {
    ...
    /// position_ids: `[3, batch, seq_len]` (or `[batch, seq_len]` broadcast to 3).
    ...
    /// freqs: `[3, batch, seq_len, half_dim]` -> `[batch, seq_len, half_dim]`.
...
/// Apply MRoPE to Q and K. q, k: `[batch, heads, seq, head_dim]`;
...
// Attention (ERNIE-4.5 projections + MRoPE).
struct Attention {
...
impl Attention {
    fn from_weights(
        ...
        let q_proj =
            UnifiedLinear::from_weights(weights, &format!("{prefix}.self_attn.q_proj"), gs, bits)?;
        let k_proj =
            UnifiedLinear::from_weights(weights, &format!("{prefix}.self_attn.k_proj"), gs, bits)?;
        let v_proj =
            UnifiedLinear::from_weights(weights, &format!("{prefix}.self_attn.v_proj"), gs, bits)?;
        let o_proj =
...
impl PaddleOcrTextModel {
    pub fn from_weights(weights: &WeightMap, config: &PaddleOcrTextConfig) -> Result<Self, String> {
        let gs = config.group_size();
        let bits = config.bits();
...
impl mlxcel_core::generate::LanguageModel for PaddleOcrTextModel {
    fn forward(
        &self,
        input_ids: &MlxArray,
        caches: &mut [KVCache],
    ...
    fn make_caches(&self) -> Vec<KVCache> {
        PaddleOcrTextModel::make_caches(self)
    }
