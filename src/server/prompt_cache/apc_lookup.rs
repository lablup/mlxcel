...
///
/// The return value is always a multiple of `block_size`, never the raw
/// input `matched_len`, even when every covered block agrees: the result is
/// `consistent_blocks * block_size`, where `consistent_blocks` is capped by
