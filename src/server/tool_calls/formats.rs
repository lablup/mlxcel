...
/// Closes the outer per-turn `message` wrapper. The wrapper's open is always
/// either primed by the generation prompt (the current turn) or written for a
/// prior turn's history, never generated by the model itself, so only the
/// close ever appears in a completion.
const K3_MESSAGE_CLOSE: &str = "<|close|>message<|sep|>";
...
/// Maximum number of `<|open|>call ...<|sep|>` blocks [`try_kimi_k3`] will
/// accept from a single model output, and the maximum number of
/// `<|open|>argument ...<|sep|>` entries accepted per call. Same defensive cap
/// as `MINIMAX_M2_MAX_CALLS` / `QWEN3_CODER_MAX_CALLS` / `MINIMAX_M3_MAX_CALLS`
/// / `KV_GRAMMAR_MAX_CALLS` elsewhere in this file: bounds memory amplification
/// under a long run of well-formed open tags from an adversarial or degenerate
/// generation, while leaving room for any real parallel-call or wide-argument
/// use.
const KIMI_K3_MAX_CALLS: usize = 1024;
const KIMI_K3_MAX_ARGUMENTS_PER_CALL: usize = 1024;

/// Structural markers [`try_kimi_k3`] counts when deciding whether an output
/// is really Kimi K3's XTML rather than another family's output that happens
/// to echo one of these strings, e.g. quoting it, or answering a question
/// about the format.
///
/// Mirrors [`try_harmony`]'s two-marker requirement (`<|channel|>` **and**
/// `<|message|>` both present, not either alone): claiming on any single one
/// of these used to be enough, so a lone `<|open|>response<|sep|>` echoed by
/// any model silently deleted everything the parser then treated as coming
/// "before" the response. A genuine K3 turn always clears two: the renderer
/// closes `response` on every turn (even a primed-open one), and always pairs
/// that with either a closed `think` block or an opened/closed `tools` block
/// alongside it, so requiring two raises the bar from "one distinctive
/// substring" to "two independently-spelled ones" without costing any real
/// K3 generation a claim.
const K3_STRUCTURAL_MARKERS: [&str; 12] = [
    K3_THINK_OPEN,
    K3_THINK_CLOSE,
    K3_RESPONSE_OPEN,
    K3_RESPONSE_CLOSE,
...
/// ```
///
/// Declines (`None`) unless at least two of [`K3_STRUCTURAL_MARKERS`] are
/// present (#1743 security review); no other family emits any of them, but a
/// single one is not distinctive enough to claim the whole output on, since
/// that also decides what gets silently discarded as coming "before" it.
/// Reasoning is reported when the think block is still in the text;
/// `parser::parse_tool_calls` strips it before this runs, so on that path the
/// reasoning comes from the shared stream filter instead and this stays
...
pub fn try_kimi_k3(text: &str) -> Option<ToolCallParseResult> {
    let marker_hits = K3_STRUCTURAL_MARKERS
        .iter()
...
fn parse_kimi_k3_calls(body: &str, calls: &mut Vec<ParsedToolCall>) {
    ...
    while let Some(at) = rest.find(K3_CALL_OPEN) {
        ...
        if let Some(name) = kimi_k3_attribute(header, "tool") {
            ...
            if calls.len() >= KIMI_K3_MAX_CALLS {
                break;
            }
        }
...
///
/// A whole-object `<|open|>json ...<|sep|>` block is honored only when it
/// precedes the first `<|open|>argument` in the body. The renderer never
/// emits both forms for the same call (`assistant_body` picks one of
/// `K3Arguments::Entries` or `K3Arguments::RawJson`), so a `json` block that
/// appears after an explicit `argument` entry is not the renderer's own
/// output; it is text a model wrote, and previously taking it as the whole
/// arguments object let a model quote attacker-controlled text into a string
/// argument and have it silently replace every argument already parsed.
fn parse_kimi_k3_arguments(call_body: &str) -> String {
    let json_at = call_body.find(K3_JSON_OPEN);
    let argument_at = call_body.find(K3_ARGUMENT_OPEN);
    let json_leads = match (json_at, argument_at) {
        (Some(json), Some(argument)) => json < argument,
        (Some(_), None) => true,
        (None, _) => false,
    };
    if json_leads && let Some(at) = json_at {
    ...
    while let Some(at) = rest.find(K3_ARGUMENT_OPEN) {
        ...
        if let Some(key) = kimi_k3_attribute(header, "key") {
            ...
            if object.len() >= KIMI_K3_MAX_ARGUMENTS_PER_CALL {
                break;
            }
        }
...
fn kimi_k3_attribute(header: &str, key: &str) -> Option<String> {
    ...
    while let Some(at) = search.find(&needle) {
        ...
        // matched inside another attribute's value or name. Any ASCII
        // whitespace counts, not just a literal space: a header split across
        // a newline (`<|open|>argument\nkey="path"`) is still one attribute
        // list, and rejecting it on the character class dropped that argument
        // while keeping the rest of the call.
...
/// Drop a `<|open|>tools<|sep|>...<|close|>tools<|sep|>` block, contents
/// included, from text that reaches a plain content-cleaning pass rather than
/// [`try_kimi_k3`]'s own tool-call extraction: a no-tools request whose
/// generation still carries a tool-call block (the request declared no
/// `tools`, so `should_parse_tool_calls` is false and `try_kimi_k3` never
/// runs), or an output the claim predicate above declined.
///
/// Matches what the streaming `CHAT_DELIMITERS` table
/// (`super::stream_filter`) does with the same two markers: `EnterToolCall`
/// then `ExitToolCall` suppress the whole span from `delta.content`. Without
/// this, the non-streaming no-tools path only strips the marker strings
/// themselves via [`super::parser::clean_content_markers`] and leaves the raw
/// call syntax inside as visible `message.content`, disagreeing with what the
/// same generation looks like streamed.
pub(crate) fn strip_kimi_k3_tools_block(text: &str) -> String {
    let mut out = String::with_capacity(text.len());
    let mut rest = text;
    while let Some(start) = rest.find(K3_TOOLS_OPEN) {
        out.push_str(&rest[..start]);
        rest = match rest[start..].find(K3_TOOLS_CLOSE) {
            Some(close_rel) => &rest[start + close_rel + K3_TOOLS_CLOSE.len()..],
            // Unclosed: nothing after the open marker is trustworthy content.
            None => "",
        };
    }
    out.push_str(rest);
...
<tool_call>{"name": "get_weather", "arguments": {"location": "Beijing"}}</tool_call>"#;
    ...
    fn kimi_k3_content_never_carries_a_marker() {
        ...
        // Two markers of the think pair (#1743 security review tightened the
        // claim predicate to require two, so a bare close alone no longer
        // suffices here even though production never hands `try_kimi_k3` this
        // shape: `parser::strip_thinking` already consumes a primed-open think
        // block, close marker included, before this function ever runs).
        let no_response = "<|open|>think<|sep|>reasoning<|close|>think<|sep|>bare answer";
    ...
    /// #1743 security review, HIGH in practice: a single K3 marker echoed by
    /// any other model must not be enough to claim the whole output. Before
    /// the fix, `try_kimi_k3` claimed on any one of `K3_THINK_CLOSE` /
    /// `K3_RESPONSE_OPEN` / `K3_TOOLS_OPEN`, and `parse_tool_calls` returns its
    /// result unconditionally, so a model quoting or discussing the format
    /// (not itself K3) would have every byte before that one marker silently
    /// discarded as if it were reasoning or tool-call structure the caller
    /// never asked for.
    #[test]
    fn kimi_k3_declines_a_lone_marker_echoed_by_another_family() {
        for text in [
            "As documented, the assistant writes <|open|>response<|sep|> to start its answer.",
            "The reasoning block always ends with <|close|>think<|sep|> before the answer.",
            "Tool calls start with the <|open|>tools<|sep|> marker in this format.",
        ] {
            assert!(
                try_kimi_k3(text).is_none(),
                "a single echoed marker must not claim {text:?}"
            );
        }
    }

    /// Two markers of the same structural pair are enough, mirroring
    /// `try_harmony`'s `<|channel|>` + `<|message|>` requirement: this is the
    /// shape a real non-thinking K3 turn with no tool call reaches
    /// `try_kimi_k3` in (the response tag pair, no tools block).
    #[test]
    fn kimi_k3_claims_on_a_genuine_response_pair_with_no_tools() {
        let text = "<|open|>response<|sep|>It is 21C.<|close|>response<|sep|>";
        let result = try_kimi_k3(text).expect("two markers of the response pair are enough");
        assert_eq!(result.content, "It is 21C.");
        assert!(result.tool_calls.is_empty());
    }

    /// #1743 security review: a trailing `<|open|>json ...<|sep|>` block after
    /// an explicit `<|open|>argument` entry is not the renderer's own output
    /// (it never emits both forms for one call) and must not override the
    /// arguments already parsed from that entry.
    #[test]
    fn kimi_k3_trailing_json_block_cannot_override_explicit_arguments() {
        let text = concat!(
            "<|open|>tools<|sep|>",
            r#"<|open|>call tool="echo" index="1"<|sep|>"#,
            r#"<|open|>argument key="a" type="string"<|sep|>legit<|close|>argument<|sep|>"#,
            r#"<|open|>json type="object"<|sep|>{"a":"stolen"}<|close|>json<|sep|>"#,
            "<|close|>call<|sep|><|close|>tools<|sep|>",
        );
        let result = try_kimi_k3(text).expect("K3");
        let arguments: serde_json::Value =
            serde_json::from_str(&result.tool_calls[0].arguments).expect("arguments are JSON");
        assert_eq!(arguments, serde_json::json!({"a": "legit"}));
    }

    /// LOW (#1743 security review): the attribute boundary test used to
    /// require a literal space, so a header wrapped onto its own line
    /// (plausible token-by-token generation) dropped that argument while
    /// keeping the rest of the call.
    #[test]
    fn kimi_k3_attribute_accepts_any_ascii_whitespace_boundary() {
        let text = concat!(
            "<|open|>tools<|sep|>",
            "<|open|>call tool=\"t\" index=\"1\"<|sep|>",
            "<|open|>argument\nkey=\"path\" type=\"string\"<|sep|>/tmp<|close|>argument<|sep|>",
            "<|close|>call<|sep|><|close|>tools<|sep|>",
        );
        let result = try_kimi_k3(text).expect("K3");
        let arguments: serde_json::Value =
            serde_json::from_str(&result.tool_calls[0].arguments).expect("arguments are JSON");
        assert_eq!(arguments, serde_json::json!({"path": "/tmp"}));
    }

    /// #1743 security review: `KIMI_K3_MAX_CALLS` bounds parallel-call memory
    /// under a long run of well-formed open tags, and stops accumulating
    /// further calls rather than dropping every call already parsed
    /// (mirroring `MINIMAX_M2_MAX_CALLS` and the other per-file caps).
    #[test]
    fn kimi_k3_caps_calls_and_arguments_per_call() {
        let mut body = String::from("<|open|>tools<|sep|>");
        for i in 0..KIMI_K3_MAX_CALLS + 5 {
            body.push_str(&format!(r#"<|open|>call tool="t{i}" index="{i}"<|sep|>"#));
            body.push_str("<|close|>call<|sep|>");
        }
        body.push_str("<|close|>tools<|sep|>");
        let result = try_kimi_k3(&body).expect("K3");
        assert_eq!(result.tool_calls.len(), KIMI_K3_MAX_CALLS);

        let mut call_body = String::from("<|open|>tools<|sep|>");
        call_body.push_str(r#"<|open|>call tool="t" index="1"<|sep|>"#);
        for i in 0..KIMI_K3_MAX_ARGUMENTS_PER_CALL + 5 {
            call_body.push_str(&format!(
                r#"<|open|>argument key="k{i}" type="string"<|sep|>v<|close|>argument<|sep|>"#
            ));
        }
        call_body.push_str("<|close|>call<|sep|><|close|>tools<|sep|>");
        let result = try_kimi_k3(&call_body).expect("K3");
        let arguments: serde_json::Value =
            serde_json::from_str(&result.tool_calls[0].arguments).expect("arguments are JSON");
        assert_eq!(
            arguments.as_object().expect("object").len(),
            KIMI_K3_MAX_ARGUMENTS_PER_CALL
        );
    }

    /// #1743 security review: the outer per-turn `<|close|>message<|sep|>`
    /// closer is in no strip table's fixed-marker pass, so it survived to the
    /// bare-marker sweep, which removed `<|close|>` and `<|sep|>` separately
    /// and left the literal word `message` glued to the answer.
    #[test]
    fn kimi_k3_strip_markers_removes_message_close_without_leaving_the_bare_word() {
        let stray =
            "<|open|>response<|sep|>answer<|close|>message<|sep|>tail<|close|>response<|sep|>";
        let result = try_kimi_k3(stray).expect("K3");
        assert_eq!(result.content, "answertail");
        assert!(!result.content.contains("message"));
    }

    /// #1743 security review: a leftover tool-call block reaching the plain
    /// content-cleaning pass (the no-tools request path, or a claim the
    /// predicate above declined) must be dropped whole, matching what the
    /// streaming `CHAT_DELIMITERS` table suppresses for the same markers.
    #[test]
    fn kimi_k3_strip_tools_block_drops_contents_and_survives_unclosed_input() {
        let closed = concat!(
            "before ",
            "<|open|>tools<|sep|>",
            r#"<|open|>call tool="t" index="1"<|sep|><|close|>call<|sep|>"#,
        ...
        assert_eq!(strip_kimi_k3_tools_block(closed), "before  after");

        let unclosed = concat!(
            "before ",
            "<|open|>tools<|sep|>",
            r#"<|open|>call tool="t" index="1"<|sep|>"#,
        );
        assert_eq!(strip_kimi_k3_tools_block(unclosed), "before ");

        assert_eq!(
            strip_kimi_k3_tools_block("plain text, no tools"),
            "plain text, no tools"
        );
    }
}
