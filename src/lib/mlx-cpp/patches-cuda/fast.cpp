...
//     unconditionally upcasting to float32. Creates epsilon as bf16 scalar,
//     avoiding type promotion and copy_v kernels in the computation graph.
//   - layer_norm() fallback: Same pattern — use out_type for intermediate
//     computation and epsilon scalar when out_type is bf16.
//
...
array rms_norm(
  ...
  // CUDA patch: Determine the compute dtype for the fallback path.
  // For bf16 inputs, compute in bf16 to avoid fp32 type promotion and
  // unnecessary copy_v kernels. For other float types, use fp32 as before.
  auto compute_type = (out_type == bfloat16) ? bfloat16 : float32;

  auto fallback =
      [has_weight, eps, out_type, compute_type, s](const std::vector<array>& inputs) {
        auto x = astype(inputs[0], compute_type, s);
        x = multiply(
            x,
            rsqrt(
                add(mean(square(x, s), -1, /* keepdims */ true, s),
                    array(eps, compute_type),
                    s),
                s),
            s);
...
array layer_norm(
  ...
  auto fallback = [has_weight, has_bias, eps, out_type, compute_type, s](
                      const std::vector<array>& inputs) {
    auto x = astype(inputs[0], compute_type, s);
    ...
    x = multiply(xc, rsqrt(add(v, array(eps, compute_type), s), s));
    x = astype(x, out_type, s);
