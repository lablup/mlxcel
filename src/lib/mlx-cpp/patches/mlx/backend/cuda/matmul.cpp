...
// Modified by mlxcel: Added general GatherMM case using CUTLASS grouped GEMM.
...
void GatherMM::eval_gpu(const std::vector<array>& inputs, array& out) {
  ...
  // [mlxcel] General case: M>1, N>1
  // Use CUTLASS grouped GEMM with GPU-side pointer preparation.
  // Each output batch element selects A and B matrices via indices.
  auto lhs_idx = ensure_row_contiguous(lhs_indices, encoder, s);
  auto rhs_idx = ensure_row_contiguous(rhs_indices, encoder, s);
