...
// MMA path selector for the quantized GEMM (lablup/mlxcel#1543).
//
// This was a bool named `SM80`, which can say "tensor cores" or "no tensor
// cores" and nothing else. Volta needs a third answer: it has tensor cores,
// but only the 8x8x4 quad-pair atom and no bf16 atom at all, so a bool put it
// on the scalar arm at 14.0 TFLOPS f32 while the card can do 95 to 100 TFLOPS
// in f16. `qmm_naive` is 88.9 percent of prefill GPU time on this hardware.
//
// The sm_80 quantized GEMM cannot simply be gated open for Volta: it needs
// `SM80_CP_ASYNC_CACHEALWAYS` and `SM75_U32x4_LDSM_N`, neither of which exists
// on sm_70. This kernel uses neither, which is why the atom swap is enough
// here and is not enough there.
enum class MmaPath : int {
  Scalar = 0, // UniversalFMA, any architecture
  Sm70 = 1,   // Volta 8x8x4 quad-pair tensor core
  Sm80 = 2,   // Ampere and later 16x8x16
};

template <MmaPath Path = MmaPath::Scalar, typename Element = float, typename CtaTiler>
inline constexpr auto make_tiled_mma(CtaTiler cta_tiler) {
  // Note: Kernel launcher assumes num_threads being same for all parameters.
  constexpr bool volta_ok =
      Path == MmaPath::Sm70 && cuda::std::is_same_v<Element, half_t>;
  constexpr bool ampere_ok =
      Path == MmaPath::Sm80 && !cuda::std::is_same_v<Element, float>;

  if constexpr (volta_ok) {
    // `SM70_8x8x4`'s ThrID is `SM70_QuadPair`, 8 threads rather than the 32 an
    // sm_80 atom uses, so a 16-way atom layout is what lands on the 128 threads
    // every other arm launches with. The launcher reads that number back to
    // size the block, and a mismatch surfaces only at dispatch as
    // `cuGraphAddKernelNode ... invalid argument`.
    using Atom = SM70_8x8x4_F32F16F16F32_TN;
    if constexpr (size<0>(cta_tiler) >= 32) {
      return make_tiled_mma(Atom{}, Layout<Shape<_4,_4,_1>>{}, Tile<_32,_32,_4>{});
    } else {
      return make_tiled_mma(Atom{}, Layout<Shape<_2,_8,_1>>{}, Tile<_16,_32,_4>{});
    }
  } else if constexpr (ampere_ok) {
    ...
    using Atom = UniversalFMA<float, Element, Element>;
    return make_tiled_mma(Atom{}, Layout<Shape<_16,_8,_1>>{});
  }
}

// Keep the bool spelling working for callers not part of this change.
// `gather_gemm.cu` still calls `make_tiled_mma<SM80, Element>` and is dead code
// in an mlxcel build, so leaving this overload means the enum cannot break a
// translation unit nobody is looking at.
template <bool SM80, typename Element = float, typename CtaTiler>
inline constexpr auto make_tiled_mma(CtaTiler cta_tiler) {
  return make_tiled_mma<SM80 ? MmaPath::Sm80 : MmaPath::Scalar, Element>(
      cta_tiler);
}
