...
template <bool KMajor, bool HasKResidue, MmaPath Path,
          typename CtaTiler,
          typename TensorA,
          typename TensorB,
          typename TensorS,
...
CUTE_DEVICE void qmm_naive_mainloop(
  ...
  // Define MMA.
  auto mma = make_tiled_mma<Path, Element>(cta_tiler);
...
template <int GroupSize, bool KMajor, bool HasKResidue, MmaPath Path,
          typename Element, typename Quant, typename Scale, typename CtaTiler>
__global__
__launch_bounds__(decltype(size(make_tiled_mma<Path, Element>(CtaTiler{})))::value)
void qmm_naive_kernel(
  ...
  qmm_naive_mainloop<KMajor, HasKResidue, Path>(
      cta_tiler,
      gA,
      gB,
      gS,
