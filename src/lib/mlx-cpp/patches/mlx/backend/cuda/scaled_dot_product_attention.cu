...
//
// Patched by mlxcel: extend the sdpa_vector decode kernels and the
// supports_sdpa_vector gate to head_dim 256 and 288, so head_dim > 128 models
// (gemma family 256, qwen3.5/3.6 256, baichuan-m1 256, paligemma2 288) take the
// fused vector path at decode instead of the unfused materializing SDPA
// fallback (see lablup/mlxcel#675). Three changes: (A) dispatch_headdim gains
// case 256/288 plus a default that throws on a gate/dispatch desync (turns a
// silent no-launch with garbage output into a loud error); (B)
// supports_sdpa_vector accepts head_dim 256/288 behind the
// MLXCEL_SDPA_VECTOR_LARGE_D env kill switch (default on) so A/B and rollback
// need no rebuild; (C) __launch_bounds__ caps the larger per-thread register
// footprint of the D=256/288 instantiations to the 1024/256/1024 thread
// launches (a no-op for D <= 128 which already fit). The .cpp is unchanged:
// cudnn rejects head_dim > 128, so widening this gate alone routes 256/288
// decode to the vector kernels. Synced to upstream e9463bb.
...
#include "mlx/utils.h" // mlxcel: env::get_var for the MLXCEL_SDPA_VECTOR_LARGE_D kill switch

#include <cooperative_groups.h>
#include <cooperative_groups/reduce.h>

#include <stdexcept> // mlxcel: std::runtime_error in the dispatch_headdim default
#include <string> // mlxcel: std::to_string in the dispatch_headdim default
...
__global__ __launch_bounds__(1024) void kernel_sdpav_1pass(
    const T* Q,
    const T* K,
    const T* V,
    T* O,
...
template <typename T, bool do_causal, int D>
__global__ __launch_bounds__(256) void kernel_sdpav_2pass_1(
    const T* Q,
    const T* K,
    const T* V,
    const T* sinks,
...
template <typename T, bool do_causal, int D>
__global__ __launch_bounds__(1024) void kernel_sdpav_2pass_2(
...
void dispatch_headdim(int n, F&& f) {
  switch (n) {
    ...
    case 128:
      ...
      break;
    // mlxcel: head_dim 256 (gemma family, qwen3.5/3.6, baichuan-m1) and 288
    // (paligemma2). Both are divisible by BD=32, so v_per_thread and the fixed
    // shared-memory tiles are the only D-dependent quantities and the existing
    // kernels handle them unchanged.
    case 256:
      f(std::integral_constant<int, 256>{});
      break;
    case 288:
      f(std::integral_constant<int, 288>{});
      break;
    // mlxcel: reachable only if supports_sdpa_vector and this dispatch ever
    // desync. Upstream had no default, so an unhandled head_dim launched no
    // kernel and left the output buffer as uninitialized garbage. Fail loudly.
    default:
      throw std::runtime_error(
          "sdpa_vector: unsupported head_dim " + std::to_string(n));
  }
}
...
bool supports_sdpa_vector(
  ...
  // mlxcel: head_dim 256/288 (the head_dim > 128 models) are gated behind a
  // static env kill switch, default on. It lets token-identity and perf A/B
  // against the old materializing fallback, and a rollback, run without a
  // rebuild. cudnn rejects head_dim > 128, so this widened gate alone routes
  // their decode to the vector kernels; the .cpp needs no change.
  static bool large_d_enabled = env::get_var("MLXCEL_SDPA_VECTOR_LARGE_D", 1);
  const bool sdpa_supported_head_dim = query_head_dim == value_head_dim &&
      (query_head_dim == 64 || query_head_dim == 96 || query_head_dim == 128 ||
       (large_d_enabled &&
        (query_head_dim == 256 || query_head_dim == 288)));
