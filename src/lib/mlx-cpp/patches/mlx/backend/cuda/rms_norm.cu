// mlxcel overlay patch (issue #824): CUDA RMSNorm kernel reverted to the
// upstream pre-#3792 implementation (ml-explore/mlx commit 1700b39a).
//
// Upstream MLX #3792 ("Fix CUDA RMSNorm small-row dispatch") and the later
// #3850 rewrite ("RMSNorm forward speed up") both regressed the small-axis
// CUDA RMSNorm for the DeepSeek-V2 MLA `kv_a_layernorm` (axis_size == the
// kv_lora_rank of 512, which with N_READS == 8 for half-precision activations
// lands on the multi-warp block_dim == 64 dispatch config). The result was
// deterministically-wrong RMSNorm output that degenerated DeepSeek-V2 text
...
//
// ---- begin upstream mlx/backend/cuda/rms_norm.cu @ 1700b39a ----
