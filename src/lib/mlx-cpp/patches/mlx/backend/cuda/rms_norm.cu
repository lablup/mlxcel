...
//
// Patched by mlxcel: `dispatch_num_chunks` gains a wide-block arm for the
// few-rows case. Synced to upstream pin 9a795735; the only delta is
// `dispatch_wide_block` and the `n_rows` argument threaded into
// `dispatch_num_chunks`. Everything else is upstream verbatim, so a future
// resync is a diff against the pinned file.
//
// Why it exists. The grid is one block per row, so a decode step (one row)
// runs the whole normalization in a single block, and upstream only ever
// chooses 64 or 128 threads for it: `axis_size <= N_READS * 64` takes 64, and
// every larger case funnels into `dispatch_chunks<128>`. Gemma 4 12B at
// hidden 3840 gets 128 threads and 4 chunks, Qwen 3.8 27B at 5120 gets 128 and
// 5. That is four warps on one SM of eighty, each thread walking 32 or 40
// elements in a serial loop, for a kernel that `rms_norm_small` runs 336 times
// per token and that measures 5.74 us a launch, 7.7% of the decode window on a
// V100 for 7.7 KB of work.
//
// Upstream's choice is right for prefill, where the grid is already wide and
// 128-thread blocks pack the machine; the comment above `rms_norm_small` about
// occupancy dropping to 30-40% is about that case. It is wrong when there is
// one row and nothing else to fill the SM with, so the arm below applies only
// when the row count is too small to fill the grid, and leaves the upstream
// path exactly as it was otherwise.
...
// [mlxcel] Rows below which one block is the whole launch and a wider block is
// the only way to put more of an SM to work. Decode is one row; a speculative
// verify block is a handful. Past this the grid fills the machine on its own and
// upstream's 128-thread blocks are the better shape.
constexpr int kMlxcelWideBlockMaxRows = 8;

// [mlxcel] Kill switch: `MLXCEL_RMS_WIDE_BLOCK=0` restores upstream's block
// choice for every shape. The wide arm changes how many threads reduce the
// axis, so it changes the reduction tree and therefore the last bits of the
// normalization; this is what an A/B on output quality selects between, and
// what a rollback uses if a shape ever regresses. Read once per process.
inline bool mlxcel_rms_wide_block_enabled() {
  static const bool enabled = []() {
    const char* e = std::getenv("MLXCEL_RMS_WIDE_BLOCK");
    if (e == nullptr) {
      return true;
    }
    std::string v(e);
...
// [mlxcel] Smallest instantiated block that covers the axis in one chunk.
//
// `nvec` is how many N_READS-wide vectors the axis needs, so a block of that
// many threads finishes in a single pass with no loop at all. The caller only
// reaches this for `axis_size <= N_READS * 128 * 8`, which is `nvec <= 1024`,
// so one of the widths below always fits and the fallback is unreachable in
// practice rather than a silent narrowing.
//
// The widths are spaced 128 apart rather than doubling, and that is the point.
// `rms_norm_small`'s fast path needs `axis_size == N_READS * BLOCK * CHUNKS`
// exactly; miss it and every load takes a bounds check. Qwen 3.8 27B at hidden
// 5120 hits it today with 128 threads and 5 chunks, and a power-of-two width
// would have quietly traded that away for the wider block. 640 keeps it, with
// one chunk instead of five. Gemma 4 12B at 3840 has no exact width at or below
// 1024 that is a multiple of 32 other than 480, and 512 leaves it exactly as
// unaligned as the 128-thread choice it replaces, so nothing is lost there
// either.
template <typename F>
void dispatch_wide_block(int nvec, F&& f) {
  constexpr auto one = std::integral_constant<int, 1>{};
  if (nvec <= 256) {
    f(std::integral_constant<int, 256>{}, one);
  } else if (nvec <= 384) {
    f(std::integral_constant<int, 384>{}, one);
  } else if (nvec <= 512) {
    f(std::integral_constant<int, 512>{}, one);
  } else if (nvec <= 640) {
    f(std::integral_constant<int, 640>{}, one);
  } else if (nvec <= 768) {
    f(std::integral_constant<int, 768>{}, one);
  } else if (nvec <= 896) {
    f(std::integral_constant<int, 896>{}, one);
  } else if (nvec <= 1024) {
    f(std::integral_constant<int, 1024>{}, one);
  } else {
...
void dispatch_num_chunks(int axis_size, int n_rows, F&& f) {
  int nvec = (axis_size + N_READS - 1) / N_READS;
  if (axis_size <= N_READS * 64) {
    f(std::integral_constant<int, 64>{}, std::integral_constant<int, 1>{});
  } else if (
      n_rows <= kMlxcelWideBlockMaxRows && mlxcel_rms_wide_block_enabled()) {
    dispatch_wide_block(nvec, f);
...
void RMSNorm::eval_gpu(
  ...
  dispatch_float_types(out.dtype(), "rms_norm", [&](auto type_tag) {
    ...
    if (axis_size <= N_READS * 128 * 8) {
      dispatch_num_chunks<
          N_READS>(axis_size, n_rows, [&](auto block_size, auto n_chunks) {
...
void RMSNormVJP::eval_gpu(
  ...
  if (has_w && cu::device(s.device).compute_capability_major() >= 8) {
    dispatch_float_types(gx.dtype(), "rms_norm_vjp", [&](auto type_tag) {
      using DataType = cuda_type_t<MLX_GET_TYPE(type_tag)>;
      constexpr int N_READS = 16 / sizeof(DataType);
      // [mlxcel] The backward pass keeps upstream's block choice: it is gated
      // on sm_80 and later, so it never runs on the hardware the wide-block arm
      // was measured for, and its `n_rows` is a training shape rather than a
      // decode step. Passing a row count past `kMlxcelWideBlockMaxRows` selects
      // the upstream branch explicitly instead of leaving it to coincidence.
      dispatch_num_chunks<N_READS>(
          axis_size,
          kMlxcelWideBlockMaxRows + 1,
