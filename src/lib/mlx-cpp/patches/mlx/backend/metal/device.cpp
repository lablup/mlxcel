...
// Patched by mlxcel: a runtime override for the per-command-buffer input
// budget. Synced to upstream 81ba1c6a. The delta is three hunks: the <atomic>
// include below, the override read in `CommandEncoder::needs_commit`, and the
// two bridge entry points `mlxcel_set_mb_per_buffer_override()` /
// `mlxcel_mb_per_buffer_override()` at the end of the file. Everything else is
// upstream verbatim, so a bump refreshes this file and re-applies those hunks.
//
// Why it exists. MLX commits a command buffer once the element count of its
// distinct inputs passes `max_mb_per_buffer_ << 20`, and `Device` latches that
// cap from `MLX_MAX_MB_PER_BUFFER` once, at construction. The right cap differs
// by phase: decode streams the whole weight set every token, so the default
// (40-50) commits a buffer every one to two layers and leaves the GPU idle at
// each boundary (+3% to +21% decode on M1 Ultra at 1000), while prefill holds
// its activations until the buffer completes, so the same 1000 roughly doubles
// peak memory on a 2048-token prompt. A process-wide constant cannot serve
// both; mlxcel raises the cap around decode steps only and leaves prefill on
// the device default. Upstream exposes no setter, so the switch lives here.
//
// Zero means "no override": `needs_commit` then behaves exactly as upstream.
// Relaxed ordering is enough because the value only decides where a command
// buffer boundary falls, never what is computed, and the caller sets it on the
// thread that encodes the work it means to affect.
...
// mlxcel: runtime override of the input budget (see the header comment).
static std::atomic<int>& mlxcel_mb_per_buffer_override_slot() {
...
bool CommandEncoder::needs_commit() const {
  ...
  if (int override_mb =
          mlxcel_mb_per_buffer_override_slot().load(std::memory_order_relaxed);
      override_mb > 0) {
    max_mb = override_mb;
  }
...
// mlxcel: bridge entry points for the input-budget override, called through
// the cxx bridge (mlxcel_core::set_metal_mb_per_buffer_override). Declared here
// rather than in a header because upstream owns every header in this tree and
// an overlay that adds one would drift on the next pin bump.
void mlxcel_set_mb_per_buffer_override(int mb) {
  mlxcel_mb_per_buffer_override_slot().store(
      mb > 0 ? mb : 0, std::memory_order_relaxed);
}
