...
pub(super) fn resample(
    ...
    if policy.resampling == AudioResamplingPolicy::Phi4MmSpeechLib {
        let (output, effective_rate) = phi4mm_resample(samples, source_rate);
        check_cancel(
            cancelled,
            AudioPreprocessCheckpoint::Resample,
            Some(clip_index),
...
pub(crate) fn phi4mm_resample(
    ...
    if down <= 1 {
        (std::borrow::Cow::Borrowed(samples), effective_rate)
    } else {
        (
            std::borrow::Cow::Owned(super::resampling::scipy_resample_poly_down(samples, down)),
            effective_rate,
        )
    }
}
