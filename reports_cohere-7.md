...
## 23. 생성 스트림 공유 (19절 3번 후보)

원인: MLX의 `new_thread_local_stream`은 부를 때마다 새 인덱스를 발급하고, 스레드는 처음 보는 인덱스마다 새 GPU 스트림(command queue, 해제되지 않음)을 만든다. mlxcel은 `CxxGenerator`와 `SpeculativeGenerator`를 만들 때마다 새 핸들을 받았다. mlx-lm은 모듈 수준 `generation_stream` 하나를 모든 `generate` 호출이 공유한다.

수정: 프로세스 전체에서 한 번만 만드는 핸들(`shared_thread_local_stream_gpu`, `streams::shared_thread_local_generation_stream`)을 두 생성기에 적용했다. 스레드마다 다른 스트림으로 풀리는 성질은 그대로다. 새 단위 테스트가 같은 스레드의 공유 핸들은 같은 스트림, 새 핸들은 다른 스트림, 다른 스레드는 다른 스트림으로 풀리는 것을 확인한다.

측정: `scripts/bench_decode.sh` pp512, 수정 전(`f86ebbc1`) 대 수정 후 바이너리, 길이별 ABBA 3라운드(arm당 6회), 인덱서 정지, 부하 평균 3.2 → 1.4. 스크립트 `.work/cohere2/stream_ab.sh`.

| 조건 | 수정 전 decode 중앙값 | 수정 후 | 차이 | prefill 수정 전 / 후 |
|---|---|---|---|---|
| tg64, 예열 20 | 107.96 (106.74 ~ 108.31) | 113.34 (111.77 ~ 114.11) | +5.0% | 702 / 727 |
| tg128, 예열 20 | 112.00 (111.90 ~ 112.23) | 114.42 (113.87 ~ 116.43) | +2.2% | 703 / 727 |
| tg256, 예열 20 | 113.98 (113.84 ~ 114.11) | 115.19 (114.97 ~ 116.25) | +1.1% | 703 / 727 |
| tg128, 예열 없음 (3회) | 113.47 / 113.43 / 113.47 | 113.59 / 112.37 / 113.64 | 차이 없음 | 약 620 / 약 620 |

- 새 스트림 비용은 decode 시작(약 20 ~ 40 ms)만이 아니라 prefill(512토큰에서 약 25 ms, 702 → 727 tok/s)에도 붙어 있었다. 17절에서 12 ms가 44 ms로 커지던 미설명분이 이것으로 보인다.
- 예열 없는 조건(프로세스 하나에 generator 하나)은 두 바이너리가 같다. 그 경우는 원래 스트림을 하나만 쓰기 때문이다.
- 그래서 실제 사용 경로의 이득은 작다. CLI `generate`는 실행당 세션 하나, `chat`은 턴이 바뀌어도 세션을 재사용하고, 서버 `BatchScheduler`는 자체 스트림 하나를 쓴다. 이득을 보는 곳은 한 프로세스에서 generator를 여러 번 만드는 경우(벤치 하네스, `speculative_bench`, 라이브러리 사용자)이고, generator마다 해제되지 않는 command queue가 쌓이던 문제도 같이 없어진다.
- 벤치 관점에서는 이것이 공정성 수정이다. `bench_mlxlm.py`도 예열 후 새 `generate`를 호출하지만 mlx-lm은 스트림을 공유하므로 이 비용이 없었다. mlxcel만 pp512/tg128에서 decode 약 2%, prefill 약 3.5%를 불리하게 재고 있었다.
- 수정 후 prefill은 727 대 mlx-lm 778.5로 격차가 약 9.8%에서 약 6.6%로 줄었다. 남은 격차가 1번 후보(prefill)의 대상이다.
- 출력 동등성: greedy 120토큰 2개 prompt에서 두 바이너리의 생성 텍스트가 같다(다른 줄은 로딩 시간과 속도 표시뿐).
- 게이트: `streams::` 테스트 18개 통과, `cargo fmt --check`, `cargo clippy -p mlxcel-core --all-targets -D warnings` 통과. 전체 workspace 테스트는 아직 돌리지 않았다.
