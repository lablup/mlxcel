...
async function clients(mode) {
  ...
  for (let i = 0; i < (mode === 'two-visible' ? 2 : 1); i++) {
    ...
    await cdp.send('Browser.setWindowBounds', { windowId, bounds: { windowState: 'normal', left: 20 + i * 710, top: 20, width: 700, height: 900 } });
...
async function request(mode, pair) {
  ...
  await writeFile(output, JSON.stringify({ status: 'in-progress', samples: results }, null, 2));
  return rate;
}
...
try {
  ...
  const summaries = [];
  for (const mode of ['one-visible', 'two-visible', 'hidden']) {
  ...
  await writeFile(output, JSON.stringify({ status: summaries.some((x) => x.status === 'investigate') ? 'investigate' : 'within-target', source: 'native completion backend timings.predicted_per_second; no browser or word-count estimate', samples: results, summaries }, null, 2));
} catch (error) {
  await writeFile(output, JSON.stringify({ status: 'failed', samples: results, error: error instanceof Error ? error.message.replaceAll(key, '[redacted]') : 'unknown' }, null, 2));
  throw error;
