...
import { Button, DataTable, DenseList, Dialog, EmptyState, ErrorBanner, Field, Inspector, LoginView, ProgressBar, SchemaMismatchView, Select, StatusBadge, Tabs, Tooltip } from './design-system/primitives';
  ...
  return (
    <div className="gallery-grid">
      <article className="surface-card">
        ...
        <Select label={t(props.locale, 'gallery.select.native')} value={props.selectValue} onChange={props.onSelectChange} options={[{ value: 'ready', label: t(props.locale, 'models.status.ready') }, { value: 'unloaded', label: t(props.locale, 'models.status.unloaded') }]} />
      </article>
      <article className="surface-card">
...
function StatePanel(props: { locale: Locale }): React.JSX.Element {
  return (
    <div className="gallery-grid">
      <LoginView title={t(props.locale, 'state.unauthorized.title')} body={t(props.locale, 'state.unauthorized.body')} tokenLabel={t(props.locale, 'login.token.label')} tokenHelp={t(props.locale, 'login.token.help')} submitLabel={t(props.locale, 'login.submit')} logoutLabel={t(props.locale, 'login.logout')} onSubmit={() => undefined} onLogout={() => undefined} testId="gallery-login" />
