...
import { cached, formatBytes, localeTag } from '../../design-system/format';
...
// Counts show at most two fraction digits; the plural form is chosen from that same rounding.
const COUNT_OPTIONS = { maximumFractionDigits: 2 } as const;

export function formatCount(value: number, locale: Locale): string {
  if (!Number.isFinite(value)) return t(locale, 'format.unknown');
  return cached(numberFormats, locale, () => new Intl.NumberFormat(localeTag(locale), COUNT_OPTIONS)).format(value);
}

/**
 * Picks the catalog key for a count: English "1 request", "2 requests"; Korean has one form.
 * The form follows the number as formatCount shows it, rounded the same way: 1.004 reads "1 request".
 */
export function pluralKey(value: number, [one, other]: UnitKeys, locale: Locale): StringKey {
  return cached(pluralRules, locale, () => new Intl.PluralRules(localeTag(locale), COUNT_OPTIONS)).select(value) === 'one' ? one : other;
}
...
export function clockTime(at: string | number | null, locale: Locale): string {
  ...
  return date === null ? t(locale, 'activity.unknown_time') : cached(timeFormats, locale, () => new Intl.DateTimeFormat(localeTag(locale), { timeStyle: 'medium' })).format(date);
}

export function dateTime(at: string | number | null, locale: Locale): string {
  const date = toDate(at);
  return date === null ? t(locale, 'activity.unknown_time') : cached(dateTimeFormats, locale, () => new Intl.DateTimeFormat(localeTag(locale), { dateStyle: 'medium', timeStyle: 'medium' })).format(date);
}
