...
- **`llama-server`-style operation.** `mlxcel-server` accepts many `llama-server`-compatible flags and `LLAMA_ARG_*` environment variables, which makes migration from llama.cpp-based scripts simpler. Treat this as compatibility-oriented, not a guarantee that every llama.cpp option has identical behavior. The verified boundary against the pinned llama-server `b10621` nightly is maintained as a machine-readable manifest in [`compat/llama-server/b10621/`](compat/llama-server/b10621/) and documented in [`docs/llama-server-compat.md`](docs/llama-server-compat.md).
