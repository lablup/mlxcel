...
- **A generation that goes entirely to the reasoning channel says so** (#1721). `mlxcel generate` suppresses the `<think>` channel by default, so a reasoning model whose generation ends before the channel closes printed a blank line and a timing line, which is indistinguishable from a broken checkpoint. Both the one-shot flow and the chat REPL now print how many tokens went to the reasoning channel and how to see them. `scripts/ab_output_equality.sh` is the output half of an A/B built on the same reading: it forces `--temp 0` on both arms so a checkpoint's own `generation_config.json` cannot turn the comparison into two samples, runs both with `--show-reasoning` so two empty content channels do not compare equal, and runs the baseline twice as a control that reports INCONCLUSIVE instead of attributing run-to-run drift to the arm under test.
...
- **`mlxcel serve` carries the help sections that only `mlxcel-server` had** (#1628). The Model store and Embeddings/Reranking groups are now in both `--help` outputs, and a test asserts that every flag named under `src/commands` appears in some `--help` (#1131), so the two entry points cannot drift apart again unnoticed.
- **The bit-width validation error names `--kv-bits`** (#1629) instead of reporting an out-of-range value without saying which flag carried it, and the four byte-size formatters that had been copied across the tree are one function (#1626).
...
- **Half-precision reductions in the bridge accumulate in f32** (#997). `max` over a bfloat16 array returned NaN for a finite input on M5 Max, about one call in six, which had failed `text_only_forward_produces_finite_logits` intermittently since 2026-08-02. Reading the same logits row back to the host found every element finite on 60 of 60 runs while the device reduction over that row reported non-finite on 28, with no overlap. Measured one variant per process over 150 forwards of `granite-4.0-3b-vision-4bit`: 23 non-finite for the plain reduction, 31 reshaped to 1-D, 20 made contiguous, 24 reduced over an axis, and 0 cast to f32, so element width is the trigger and contiguity is not. `sum_all` fails the same way at 28 of 150, so `max_all`, `min_all`, `sum_all`, `mean_all` and their axis forms all promote to f32 and restore the input dtype. Four hypotheses were eliminated by measurement first, and two traps are recorded in the code: timing several variants in one loop hides the fault because each `eval` synchronises the next, and a bridge `.cpp` change can leave `cargo build` reporting a no-op, so the first verification ran against a binary without the fix.
- **Youtu-VL emits patches in merge-block-major order** (#1610). #1600 corrected the un-reordering after the windowed tower; the processor was still emitting patches in raster order where the tower's rotary positions and the patch merger both expect merge-block-major, so the family still described images wrongly. Both halves are now aligned.
- **An absolute `file://` URL resolves under `--media-path`** (#1612). Since #1481 confined request media to the media root, an absolute path returned HTTP 400 rather than being probed under the root, which is what b10621 does. The leading separator is stripped and the remainder joined onto the root, so an absolute path is probed and reported missing while a relative one resolves.
- **`recurrent_gemma` builds every layer and gets its conv and RoPE layouts right** (#1687). The family was registered and could not generate text at all.
- **`nemotron_nas` resolves llama3 RoPE scaling through the shared reader, rotates after the head transpose, and reads `eos_token_id` from the config** (#1687, #1688). The family had never run. The rotation-order fix alone was not enough: the remaining defect read only `factor` out of `rope_scaling` and applied `1 / factor` as a uniform position scale, which is the `linear` scheme, to a checkpoint declaring `rope_type: "llama3"` whose rescaling is piecewise in the frequency. Before the first fix the model emitted a single "I"; after it, a fluent loop; only with both does it answer, and it measures 19.37 against mlx-lm's 19.55 tok/s on the same machine. The verification that first passed it was a 1300-token needle test, which cannot reach the branch: `original_max_position_embeddings` is 8192 and the llama3 low-frequency band only carries positions past it. Re-run at 14737 tokens the fixed arm answers and the reverted arm does not.
- **Inkling expands KV heads for GQA under the per-head mask** (#1549).
- **MiniCPM-V upscales an image that arrives below the scale resolution** (#1684), which had been giving it 16 vision tokens against the reference 64.
- **Phi and StableLM carry the upstream f32 attention guard** (#1710). Upstream widens the queries in phi and the queries and keys in stablelm; mlxcel had ported that only into `phixtral.rs`. `phi-2-4bit` went NaN at layer 29 of 32 in f16 and then emitted `!` for every token, reproducible in bare MLX with no mlxcel in the loop. The guard widens what upstream widens and no more: the values are only ever multiplied by probabilities in [0, 1] and cannot overflow, so widening them doubled the V read on every decode step for nothing.
- **CUDA JIT kernel configuration is serialized** (#1566), closing a divide-by-zero when two threads configured a kernel at once.
- **The real-model integration tests resolve checkpoints again** (#1722, #1726). `repo_model_dir` looked only under `models/<name>`, so none of the 30 checkpoint names resolved under the current store layout and every real-model test skipped, reporting `2 passed ... finished in 0.00s`. The root is now probed, and 15 names that changed in the store consolidation are corrected across 55 sites. A green run of a gate that resolves nothing is not evidence, and elapsed time is what separates the two.

### Performance

A dtype-promotion class runs through most of this section. A helper that builds its constants as f32 returns f32 for a half-precision input, the widened activation enters the residual stream at the first layer that uses it, and every later matmul promotes its own weight to match. Decode is bandwidth-bound, so the model reads about twice the weight bytes. Nothing is arithmetically wrong, which is why unit tests, output comparisons and perplexity all pass over it; only a dtype assertion or a same-host comparison against the reference sees it. The invariant is that a helper returns the dtype it was given, and it cannot be audited by grep, because promotion inside a compiled graph is free and promotion that escapes is not.

- **The bridge's activation helpers keep the input dtype** (#1709). Five non-compiled helpers built their constants as f32 scalars: `gelu`, `gelu_approx`, `relu`, `leaky_relu` and `compiled_gelu_mlp_forward` with its op-at-a-time fallback. M1 Ultra decode at pp512/tg128, with the same-host mlx-lm figure in brackets: `gpt_bigcode-santacoder` 51.23 to 164.63 tok/s, 28% to 90% [183.38]; `pythia-1b` 61.34 to 190.60, 31% to 97% [195.76]; `moondream2` 45.11 to 149.86; `phixtral-4x2_8-4bit` 73.12 to 84.99, 78% to 90% [94.11]. Output is byte-identical to mlx-lm on santacoder, pythia-1b and phixtral. Four controls that do not reach a fixed helper are unchanged, and `gpt2` is the mechanism's control: same code path, f32 weights, nothing to promote, 208.66 to 209.55.
- **The multimodal and 3D rotary paths restore the input dtype** (#1714). `cos` and `sin` are built in f32 on purpose and the rotated result was returned without a cast, so `q` and `k` left the function as f32 and carried through attention and the output projection. Upstream does the same widen-then-restore and ends with `q_embed.astype(q.dtype)`; mlxcel took the arithmetic and dropped the restore, in `qwen2_vl.rs`, `qwen3_vl.rs` and `qwen3_vl_moe.rs` alike. It only bites once an image has been seen, since the text-only fast path uses plain 1-D `fast_rope`, which is why the same checkpoint measured at parity on text and far below it on the image benchmark. Worth up to 2.45x on M1 Ultra and 3.26x on M5 Max, largest on the non-quantized checkpoints. Verified against mlx-vlm on three solid colours rather than one, because a single fixture cannot separate "reads the image" from "guesses the usual answer".
- **Gemma3n loads uniformly bf16** (#1709). The load policy kept the language MLP bf16 and converted everything else to f16, and MLX promotes f16 met with bf16 to f32, so one checkpoint held two dtypes and every boundary between them promoted. M5 Max decode moves from 38.02 to 47.48 tok/s and prefill from 2143 to 2371, taking decode from 78% of mlx-vlm to 97%. This is not a claim that bf16 beats f16: on families with no forced-bf16 subset the two land within noise, `qwen2.5-0.5b-bf16` at 401.8 against 397.3 and `llama-3.1-8b-bf16` at 33.07 against 33.02. The mixture is what costs, so every other family keeps converting. `MLXCEL_KEEP_BF16` is the A/B instrument.
- **Mistral 4 and Ministral 3 cast the attention scale to the query dtype** (#1711), and **GraniteMoeHybrid runs its gated RMSNorm in the input dtype** (#1723). The Granite change is 1.079x and 1.184x on `granite-4.0-h-350m-4bit` (M1 Ultra, M5 Max) and 1.044x and 1.046x on `granite-4.0-h-tiny-4bit`, every range disjoint over five runs per arm. The arms are not byte-identical, so the teacher-forced logit trace judges it: 0 of 156 disagreements at decided positions at decode width and 0 of 386 at prefill width 256, against a self-traced control of 0 of 400.
- **The GQA repeat is gone from the fused SDPA paths** (#1686), in the shared attention helper and in HunyuanVL and Ernie4.5MoeVL, Qwen2-VL takes the text-only path when no image is in play, and grouped softcap-GQA decode is on by default.
- **The Mamba2 hybrids drop two M5-gated `eval` boundaries** (#259, #266, #1685), which had been costing every host to work around a fault that #997 has now located and fixed at its source.
- **Qwen3 MoE batches the decode forward instead of issuing one forward per row** (#1616).
- **`apply_token_bias` no longer synchronizes the device on every decode step.** It read the pre-bias argmax id back to the host to feed two suppression counters, which breaks the async lookahead pipeline the decode loop is built on and costs the same regardless of vocabulary size. Measured on M1 Ultra as the delta between `--ignore-eos` and no flag: +65.4% decode time on `qwen3-0.6b-4bit`, +30.4% on `llama-3.2-1b-4bit`, +24.0% on `qwen3-30b-a3b-4bit`, +13.6% on `llama-3.1-8b-4bit`. Behind `MLXCEL_LANG_BIAS_COUNTERS=1` the same deltas are -0.6%, -1.2%, -0.4% and -1.0%. The op also stops uploading a vocab-sized f32 vector per step to bias a handful of ids. Nothing is affected unless a request or server opts into `logit_bias`, `ignore_eos` or `--lang-bias`; language steering was the worst case, since the map stays resident for the whole session.
- **Pre-Ampere CUDA gets three arms it did not have.** bf16 weights convert to f16 at load (#1542), since no pre-Ampere part has a bf16 ALU. `qmm_naive` gets a Volta tensor-core path (#1543) built on the `SM70_8x8x4_F32F16F16F32_TN` atom, bounded by tile area rather than `tile_m` because the register file is what binds: prefill goes from 138 to 536 tok/s on `qwen3.8-27B-4bit` and 326 to 1579 on `gemma-4-12B-it-4bit`, against scalar-path ceilings of 259 and 583. And `qmv` blocks output rows per warp below Ampere, which divides activation traffic by the block factor while leaving weight traffic and the per-row arithmetic untouched: `gemma-4-12B-it-4bit` decode 44.7 to 57.4 tok/s and `qwen3.8-27B-4bit` 20.6 to 24.9, generated text byte-identical at `--temp 0`. The controlled 4-bit against 8-bit pair that motivated it puts 84% of a 4-bit launch in work that does not scale with weight bytes, so decode below Ampere is not weight-bandwidth bound.

### Dependencies

- `uuid` 1.24.1 to 1.26.0 (#1529), and `hyper` 1.11.0 to 1.11.1 with `smallvec` 1.15.2 to 1.16.0 (#1682).
...
[v0.7.0]: https://github.com/lablup/mlxcel/compare/v0.6.0...v0.7.0
