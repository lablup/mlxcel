...
- **The 2D-parallelism error no longer tells the user to pass a flag that does not exist** (#1112). Rejecting a `--tp-size > 1` run that lacks a pipeline topology asked for `--tensor-parallel-size > 1`, which no binary has ever accepted, so following the instruction produced `error: unexpected argument '--tensor-parallel-size' found`. The message now names `--tp-size`, the real flag. The wrong name was also in the 2D integration test, where `pp_tp_2d_validator_accepts_combination` passed vacuously: clap rejected the flag before the validator it claimed to exercise ever ran. The test now uses `--tp-size` and fails if the arguments are rejected at parse time.
