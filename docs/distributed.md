...
- A prefill node connects to the address in the `decode_target` field of the incoming request frame to deliver the KV cache handoff, which encodes the prompt. The router populates that field only from its own configured node registry, so under the trusted-segment assumption the addresses are operator-controlled. As defense-in-depth the prefill node validates `decode_target` against an allowlist before connecting: a target outside the allowlist is rejected (the request fails with a clear error and the KV cache never leaves for an unknown address) instead of connected to, so a forged frame from a breached segment cannot redirect the handoff off-cluster. The allowlist is the prefill node's full `--decode-peers` set.

  CRITICAL configuration requirement: to keep router-driven decode balancing (the multi-node load spread added in issue #201) working, configure each prefill node's `--decode-peers` with the SAME full decode pool the router routes over (the shared cluster config), not just the prefill's own paired decode node. The router may hand any prefill node a `decode_target` for any decode node in the pool, so a prefill whose local `--decode-peers` lists only a subset would reject the router-balanced targets it has not been told about. When a prefill node has no decode allowlist configured at all (an empty set, which a correctly configured prefill-only node never has because `--decode-peers` is required), it stays permissive and logs a warning rather than rejecting, so a missing allowlist source never silently disables balancing. A request frame without `decode_target` (an older router) still falls back to the prefill's first static `--decode-peers` entry, which needs no validation because it is operator config rather than a wire-supplied address.
