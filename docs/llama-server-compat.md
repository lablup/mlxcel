...
The same issue closed the batching and slot-count entries, and #1815 finished the upstream auto-context behavior: `--parallel` accepts b10621's whole domain with `-1` (auto) as the default, resolving to 4 slots exactly as upstream's own auto does, and auto enables `kv_unified` by default. Under unified context budgeting every slot sees the whole `--ctx-size` window while the scheduler enforces one shared live-token budget across the active sequences. An explicit `--parallel N` or explicit `--max-batch-size` keeps split per-slot windows unless `--kv-unified` is also explicit; `--no-kv-unified` forces split windows. `--batch-size` is classified `aliased` onto `--prefill-chunk-size` (same logical prefill batch, different default: 512 against upstream's 2048); `--ubatch-size` is `not_applicable` (no physical micro-batch exists on unified memory; the value is accepted, noticed at startup, and reported in `/props`); and `--swa-full` is `not_applicable` and refused when set, because sliding-window families build their own ring caches from the checkpoint's `sliding_window` and the state operations the flag purchases upstream are gated on scheduler-owned caches, not ring size. The `-np`, `-kvu`, `-no-kvu`, and `-ub` single-dash spellings rewrite through the argv pre-pass.
...
`GET /props` is mounted unconditionally, as upstream mounts it, and reports the b10621 key set: `default_generation_settings` is now the upstream `{params, n_ctx}` shape rather than a flat map (**a client that read `default_generation_settings.temperature` must read `default_generation_settings.params.temperature`**), and the payload carries the model identity (`model_alias`, `model_ftype`, `model_path`), `modalities`, `media_marker`, the three endpoint toggles, `ui` / `ui_settings`, `chat_template` and `chat_template_caps`, `bos_token` / `eos_token`, `build_info`, `is_sleeping` and `cors_proxy_enabled`. When `--ctx-size 0` derives the model window, `n_ctx` is the same positive effective per-slot value reported by `/slots` and `/v1/models`; in unified mode it is the whole configured window for every slot rather than a divided share. mlxcel's own resolved-configuration keys stay as top-level extensions (`kv_unified`, `kv_cache_mode`, `kv_bits`, `speculative`, `capabilities`, and the context/batch geometry under `geometry`). `--props` now gates `POST /props` alone, which acknowledges `{"success": true}` without changing anything, exactly upstream's handler.

`GET /slots` reports b10621's slot objects from a real per-request slot registry: a generation request occupies the lowest free of `--parallel` slots from scheduler prefill admission or, for backends with no prefill event, its first decoded token until it finishes. `id`, `is_processing`, `id_task`, the prompt-token split (`n_prompt_tokens`, `n_prompt_tokens_processed`, `n_prompt_tokens_cache`), the resolved `params`, and the `next_token` progress block are live values on chat completions, completions, Responses, Anthropic messages, and native completion routes. During prefill `n_prompt_tokens_processed` climbs as processed minus cached tokens; during decode and after finish `n_prompt_tokens` is prompt plus accepted decoded tokens. `prompt` and `generated` stay redacted unless `LLAMA_SERVER_SLOTS_DEBUG` is set, which is b10621's own debug gate. A disabled endpoint (`--no-slots`) answers upstream's 501 diagnostic instead of the former 404, as do `/metrics` without `--metrics` and the slot actions without `--slot-save-path`. A slot is claimed on the request's first progress signal rather than when the route accepts it, preserving the rule that a request still waiting in the scheduler queue holds no slot. What remains recorded as a deliberate difference is the `params` key subset, which follows `/props`: a key mlxcel does not act on is omitted rather than invented.
...
### Web UI, tools, MCP, CORS proxy, and agent mode (#1435, #1838)

Everything in this group exists for b10621's embedded browser UI: `--ui` / `--webui` serve the SvelteKit bundle at `/`, `--path` replaces it with a directory, `--tools` and `--tools-runtime` expose server-executed tools to that UI through `/tools` (upstream's own developer documentation marks the endpoint UI-internal), `--mcp-servers-config` / `--mcp-servers-json` feed the same endpoint from stdio MCP child processes, `--ui-mcp-proxy` opens the generic `/cors-proxy` URL proxy so the browser can reach remote MCP servers, and `--agent` turns the proxy and every tool on at once. mlxcel accepts `--ui` / `--webui` as aliases for its bundled WebUI, serves the shell at `${api-prefix}/webui/`, and keeps `/` as the health endpoint. It still does not execute server-side tools, spawn MCP children, serve arbitrary static directories, or expose the generic CORS proxy.

Concretely, on both server binaries: `--ui` / `--webui` enable the bundled WebUI, `--no-ui` / `--no-webui` disable it, and `LLAMA_ARG_UI` resolves with b10621's `parse_bool_value` rules and the `LLAMA_ARG_NO_UI` alias. CLI flags take precedence and repeated enable/disable forms use last-wins parsing. Unsupported adjacent forms (`--ui-config`, `--ui-config-file`, `--path`, `--tools`, `--tools-runtime`, `--mcp-servers-config`, `--mcp-servers-json`, `--ui-mcp-proxy`, `--webui-mcp-proxy`, `--agent`, and the single-dash `-ag`) still fail startup with a one-line diagnostic naming the supported alternative, before the model load. Negative forms for unsupported surfaces (`--no-agent`, `--no-ui-mcp-proxy`, `--no-webui-mcp-proxy`) remain inert.
...
Capability comes from the same `config.json`-only model-type probe that already decided video support, so a newly ported VLM family is admitted with no list to update. The worker keeps its own copy of the refusal, so a path that reaches it without passing a route gate fails rather than degrading to text. The hint half of upstream's sentence is replaced on purpose: telling an operator to provide an `mmproj` has no meaning for an integrated checkpoint. The five prompt-inspection routes run the same gate for a further reason: they render through the call the generating routes render through, which resolves every media part, and then never reach a worker at all, so without a gate a text-only checkpoint would fetch a URL a client named and answer nothing but a token count for it.
...
**The resolution rule, stated once.** A request's path is resolved relative to the root first, exactly as b10621 does, and an absolute path is accepted as a fallback when it names a file inside the root. Under `--media-path /srv/media`, the relative form `file://cat.png` and the absolute form `file:///srv/media/cat.png` both read `/srv/media/cat.png`, and a bare `cat.png` does too. `file:///etc/passwd` is refused as an escape. The 255-byte cap in b10621's name validation applies to the whole path and runs before either form is tried, so a file deep enough that its absolute path exceeds 255 bytes has to be named relative to the root; that is upstream's rule, kept as it is. `mlxcel-server --help` states the same rule on the `--media-path` entry.
...
- **An absolute path inside the root, as a fallback.** Concatenation alone means the absolute form every other tool accepts can never resolve: `file:///srv/media/cat.png` is probed at `<root>/srv/media/cat.png` and reported as missing, while the file it names is present and readable. When the concatenated candidate does not resolve and the reference is itself absolute, mlxcel canonicalizes the reference and puts it through the same containment check, so the file inside the root is read and one outside it is refused as an escape (issue #1612). The fallback is gated strictly on the reference being absolute, so a relative reference never canonicalizes against the server's working directory, and the concatenation stays primary, so nothing that resolved before resolves differently now.
...
The absolute-path fallback is a divergence too, and is recorded as one for both of the things a client can observe. Upstream reports `file does not exist or cannot be opened` for every absolute spelling, inside the root or outside it; mlxcel reads the one inside and answers the one outside with `file path escapes the --media-path root`. And where a path genuinely does not resolve, mlxcel's sentence carries a trailing clause naming `--media-path` as the directory paths resolve against, which upstream's does not. The candidate actually probed goes to the server log at `debug` and never into the response, because it spells out the configured root.
...
`--slot-prompt-similarity` (`-sps`), `--cache-idle-slots`, `--ctx-checkpoints` (`-ctxcp`, also spelled `--swa-checkpoints`) and `--checkpoint-min-step` (`-cms`) are declared on both server binaries and accept their inert value; a value that asks for the behavior fails startup with a diagnostic naming what is missing. Each tunes per-slot retained prompts or a ring of context checkpoints, and mlxcel has neither: reuse goes through a process-wide radix trie over token prefixes that every request consults regardless of which sequence last held them, and at most one history-boundary snapshot is captured per sequence. `--slot-prompt-similarity` makes the point sharpest: its upstream default is `0.10`, not `0`, so accepting it inert would mean honoring a script that passes the upstream default while no slot-selection policy exists to honor. Before #1473 they were not declared at all, and clap's unknown-argument error told an operator nothing about why.
...
### Reproducible greedy output: send `cache_prompt: false`

A prompt-cache hit is a split prefill: the cached prefix was computed in an earlier forward and only the remainder is prefilled now. The split changes the floating-point summation order (the quantized matmul kernels choose their path by row count, and attention sees a different query/key shape), so the logits of the first generated token are close to a cold prefill's but not always bit-identical. On command-r7b 4-bit and Llama 3.1 8B 4-bit on M1 Ultra, splitting a 224-token prompt anywhere in its last 64 tokens moved the first token's logits by up to 0.012 to 0.031 while leaving its argmax unchanged. When a later near-tie flips, a greedy reply diverges from the cold one, so the same request can answer differently depending on how much of it the cache held. This is not cache corruption, and no split size makes it disappear; the same-width arms of the differential gate above agree exactly on `qwen3-0.6b-4bit`, which is a property of that model and prompt rather than a guarantee for every family.

When a request needs output that does not depend on the cache state (regression tests, evaluation runs, comparing two builds), send `cache_prompt: false`. The request is prefilled cold and neither reads nor donates cache entries, on the chat routes and on native `/completion`. `--no-cache-prompt` does the same server-wide.
