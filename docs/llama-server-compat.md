...
`--sleep-idle-seconds` is the one entry in this area still `deferred`. Upstream's sleep destroys the context and the model after the idle window and serves `/props`, `/models` and `/metrics` from cached responses until a generation request wakes it; mlxcel's single-model worker is created at startup and has no teardown-and-reload lifecycle to drive from an idle timer, and the router-mode pool's `unload` is the nearest thing in tree. `/props` truthfully reports `is_sleeping: false`.
