...
21. `embeddings.md` - the `/v1/embeddings` endpoint and `mlxcel embed`: embedding-checkpoint detection rules, pooling modes and the `1_Pooling/config.json` reader, L2 normalization and `dimensions`, `max_length` derivation, the padding-aware attention-mask builders, the request and response schema with error codes, the server flags, and the checklist for registering a new embedding family.
