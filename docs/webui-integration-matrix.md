...
| Browser visual/accessibility engines | `pnpm --dir webui run browser` keeps the approved Chromium pixel-baseline/Chromium-AX suite, while `pnpm --dir webui run browser:all` uses `playwright.engines.config.ts` for Chromium/Firefox/WebKit DOM, keyboard, layout and axe coverage without invoking Chromium-only CDP or screenshot baselines. CI installs all three engines and runs both paths through `verify-webui-frontend`. | Root PASS on mock/Vite evidence only: 28 default Chromium tests and 7 cross-surface/performance Chromium tests in `integration-1848/engines-f873-quiet-pass` and `chromium-prefix-default-pass`; 311 Vitest, 17 Node, 49 contract tests, typecheck and lint also passed. | Native Safari on macOS 27, manual VoiceOver, real page zoom 200%, installed-artifact browser evidence, and Chromium pixel-baseline review remain distinct evidence rows; WebKit CI/axe and Vite evidence do not replace them. |
