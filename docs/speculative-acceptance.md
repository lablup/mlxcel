...
request whose sampler is not argmax (`temperature > 0` with `top_k != 1`) to
classic decode with one warning, rather than serving the biased greedy stream
under a sampling request. At `temperature == 0` it is exactly greedy target
decoding, subject to the block-versus-chain exactness probe below.
...
**A real DFlash or DSpark drafter is rejected before the load.** The
auto-detected `dflash` above is the `DEFAULT_DRAFTER_KIND` fallback applied to
an ordinary small full model, which is exactly what the classic
`SpeculativeGenerator` wants, so that pairing keeps working. An actual DFlash
drafter checkpoint is a different object: it ships no `embed_tokens` and no
