...
| `MLXCEL_MAX_BATCH_PREFILL_TOKENS` | unsigned integer tokens, `0` disables | derived (`max_batch_prefill * prefill_chunk_size`) | Padded-token budget bounding one server batched prefill's transient memory (issue #715). The batched path pads a cohort to its longest prompt `L` and materializes a `[B, L, L]` FP32 mask, an `O(B*L^2)` transient; this caps the drained window by total padded tokens (`rows * L`) so the mask stays within `~2*N^2` bytes, with rows past the budget spilling to the chunked single-sequence path. `--max-batch-prefill-tokens` takes precedence. Unset derives `max_batch_prefill * prefill_chunk_size` (the shipped `4 * 512 = 2048`); `0` disables the cap (pre-#715 unbounded behavior). Only affects families with `supports_batched_prefill()` under `--max-batch-prefill > 1`. |
