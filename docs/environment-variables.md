...
The preferred activation surface is engine-level: detection is **default-on for the Gemma 4 family with no configuration required**, so a downstream serving app needs no setup and end users see no toggle. Since issue #967 that default-on is scoped to the requests where the collapse actually occurs: a request whose tool declarations reach the rendered prompt (`tools` non-empty and `tool_choice` other than `none`), or that asks for `response_format: json_schema` and therefore decodes under a grammar constraint. Plain chat and plain completion keep the disabled baseline, because running the detector there cost more in false positives than it bought in protection (a markdown table alignment row repeats the same 3-token cell once per column). A collapse without an amplifier is still possible, since the defect is upstream in the weights; `MLXCEL_LOOP_DETECTION=on` and the per-request fields below are the recourse, and neither is gated. The per-request fields and the global env var below are additional tuning surfaces, not the only way to turn it on, and neither is subject to that scoping.

| Variable | Values | Default | Notes |
|----------|--------|---------|-------|
| `MLXCEL_LOOP_DETECTION` | `off`/`0`/`none`/`false`/`disabled`, `on`/`default`/`true`/`enabled`, or `MIN,MAX,COUNT` (also `MIN:MAX:COUNT`) | unset | Global operator override for any model. Unset lets the Gemma 4 family default-on apply to prompt-rendered-tool and grammar-constrained requests (everything else stays disabled). `off` force-disables for every request, including the Gemma 4 family; `on` forces the recommended threshold `1,20,12` for every model and every request, plain chat included; an explicit triple (e.g. `1,20,12`) sets exact values. Set `1,20,4` to restore the pre-#967 threshold. A malformed value warns and is ignored. |
...
3. **Gemma 4 family default-on, gated on the request.** When the loaded model is in the Gemma 4 family (`Gemma4`, `Gemma4VLM`, `Gemma4Unified`) **and** the request's tool declarations reach the rendered prompt or it decodes under a `json_schema` grammar constraint, the conservative threshold `min_pattern_size=1, max_pattern_size=20, min_count=12` is applied. The tools half of that signal is read from the same helper the chat template uses to pick which tools to render, so `tool_choice: "none"` does not count: it renders no declarations, leaving a prompt identical to plain chat. The grammar half is independent of `tool_choice`. Plain Gemma 4 chat, plain `/v1/completions`, and the native `/completion` endpoint fall through to step 4.
4. **Disabled.** The default for every non-Gemma-4 model and for every unamplified request, preserving the exact baseline output.
