...
| block 6 | 3 | 21.06 (20.62 to 21.30) | 0.72x | 1.67 | 126.6 | 92.5 (91.2 to 94.2) | 30.4 | 3.7 | 1.00 to 1.26 |
...
## Limits of this fix and hand-offs

- **Measured against sharing the path.** Measured here: the Laguna pairing (0.52x to 1.13x at block 2, 0.70x to 1.33x at block 4) and Qwen 3.5 as the cross-family control (1.26x and 1.35x, text byte-identical to classic; head_dim 256, so it never enters cuDNN and the gate cannot reach it). Every other CUDA model whose masked calls with 2 to 32 query rows and prior context reach `scaled_dot_product_attention.cpp` on Ampere or later shares the new path without a number: the speculative verify of any head_dim-128-or-less pairing (in tree: Muse Glimmer DFlash, LFM2 and LFM2.5 DSpark, Inkling MTP, none re-probed here), a short incremental prefill over a reused prefix-cache prefix (2 to 32 new rows over a long `k_len`, a common server shape and unmeasured; every arm here used a 152-token prompt), and the trailing short chunk of a chunked prefill. Their exactness probes may flip a tie (a decline, not corrupted output) and their round cost was not measured. The generic `DFlashAttention::forward` passes no mask, so a non-Laguna DFlash drafter with head_dim 128 or less would still rebuild one plan per round; no such pairing is on main.
- **The fallback's cost grows with `k_len`.** The gate replaces about 66 ms of constant host time per round with GPU work that scales with the key length (the score matrix is `[B, heads, q_len, k_len]`): about 10 ms more per round at block 2 with 350 keys. Scaling that by the layers and heads puts the point where cuDNN's constant plan cost would be cheaper in the tens of thousands of key tokens at block 16; no arm here ran past 350 keys, and the bound is by `q_len` only. The chunk budget (`MLXCEL_ATTENTION_CHUNK_BUDGET_MB`, 1 GiB) now covers the diverted calls that go through `layers::attention` and `causal_attention`; the four sinks paths that call `fast_scaled_dot_product_attention_with_sinks` directly (Laguna's sliding layers, gpt_oss, deepseek_v4, falcon_ocr) bypass that budget as they did before, which is bounded for Laguna by its 512-key window and unbounded on gpt_oss's full-attention sinks layers.
- **The general fix is not attempted here.** MLX's cuDNN plan cache keys on the exact key length and mask shape, so a verify round structurally never hits it; padding or bucketing the key length (as MLX's own `use_cudnn_for_decoding` path already does for one-row calls against a fixed-size cache) would let the cache hit and keep cuDNN's faster flash kernel, recovering the 10 ms the fallback costs. That is the cache-key defect underneath this issue and the maintainer is filing it as its own issue; this change deliberately routes around it instead. The same cache's lifetime miss counter, which aborted every block 2 profile past about 170 rounds with MLX's `Cache thrashing` throw, is the evidence that the miss path is not merely slow but terminal, and the `MLX_CUDA_SDPA_CACHE_SIZE=2000` default only buys a larger lifetime budget (per MLX eval thread, since the cache is `thread_local`; a 2000-entry cache of built cuDNN graphs has no measured memory figure).
