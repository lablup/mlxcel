...
Resolved (was a separate, pre-existing bug; fixed in issue #718): `--max-kv-size N` with a prompt longer than N used to produce degenerate output on the DENSE batched-decode backend. It was NOT Int8-specific: `--kv-cache-mode fp16 --decode-storage-backend dense --max-kv-size 256` on a 520-token prompt produced the same garbage, while `fp16` on the default paged backend (where the dense front-trim is a no-op and capacity is a pool-side concern) stayed coherent. Int8 always uses the dense backend, so it always exposed this path. The root cause was attention-sink loss, not a position bug: the dense `trim_front` dropped the oldest tokens starting at position 0, discarding the leading BOS/sink tokens a transformer dumps its excess attention onto, and without them decode collapses into repetition (the StreamingLLM effect). The runtime offset/live-window bookkeeping was already correct (offset stays monotonic, RoPE positions stay aligned). The fix pins a small attention-sink prefix when trimming: `enforce_max_kv_size_for` now calls `KVCache::trim_front_keep_sink(excess, 4)`, which keeps the first 4 tokens and drops the `excess` tokens after them, mirroring upstream mlx-lm `RotatingKVCache(max_size=max_kv_size, keep=4)`. On llama-3.1-8b-4bit with a 504-token prompt and `--max-kv-size 256`, both `--kv-cache-mode fp16 --decode-storage-backend dense` and `--kv-cache-mode int8` now answer the prompt coherently, matching the paged control.
