...
| TTFT, qwen | ~13 s | 24.94 s at a 54-token prompt, warm cache, after load | **did not reproduce**, and not explained |
...
**TTFT did not reproduce, and the difference is unexplained.** The audit reports ~13 s; the figure measured here is 24.94 s at a 54-token prompt on `qwen3.8-27B-4bit`, of which 18.41 s is the fixed intercept, with the model already loaded. The obvious candidate, a cold PTX cache in the audit, is ruled out by the direction: a cold cache is slower, not faster, and every CUDA API cost in the audit's own profile is higher than this record's at the same or similar call counts (`cuModuleLoadDataEx` 848 ms against 172 ms on 13 calls both times, `cudaGraphInstantiate` 2.14 s against 1.22 s, `cudaMemcpyAsync` 4.36 s against 3.92 s on 1,847 calls both times). A slower profile with a faster reported TTFT is not consistent, so the two numbers are most likely measuring different things: the audit's prompt length, whether a chat template was applied, and whether model load was included are all unrecorded. This record's figure is defined precisely (the `Prefill:` line of `--profile` at `-n 1`, model already loaded, warm PTX cache, prompt length stated) so that the next person can compare like with like. Treat the ~13 s as unreproducible rather than as a target.
...
| #1544 | grouped GEMM arch tag on an sm_70 part | MoE 4-bit 38.61 ms/tok, 8-bit 34.29 ms/tok | | |
