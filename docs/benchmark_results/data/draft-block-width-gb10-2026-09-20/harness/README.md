...
**`MLX_CUDA_ARCHITECTURES=121` is pinned for the build and for every server.** `build.rs` auto-detects `121a` on this host. The shipped release uses plain `121` (`.github/workflows/release.yml`), and so does every earlier GB10 record here. The suffix does not change the decode kernels: in the pinned MLX tree only the NVFP4 weight-quantization converter keys on `__CUDA_ARCH_SPECIFIC__`. The pin is so the measurement describes the binaries users run.
