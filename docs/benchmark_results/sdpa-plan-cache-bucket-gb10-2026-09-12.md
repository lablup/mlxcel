> **Superseded by [`sdpa-plan-cache-bucket-gb10-2026-09-17.md`](sdpa-plan-cache-bucket-gb10-2026-09-17.md).** This record was measured on the pre-rebase tree `60341873`. Its mechanism findings (which key fields move, the three shape classes, the design and the identity argument) still hold. Its throughput table does not: re-measured on `0ef0a1a4` the comparison moved, and bucketing is behind #1817's ops fallback at three of five block widths with disjoint ranges rather than a wash. The 16k rows referenced here as suggestive remain n = 1 to 2 and were never re-run, because the re-run exceeded this host's driver budget. Do not cite the throughput numbers below.
