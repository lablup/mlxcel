...
|-------|--------|----------|--------|------|
| SmolLM-135M | 135M | 374.92 | 905.24 | 643.04 |
| ERNIE-4.5-0.3B | 300M | 495.71 | 1053.87 | 682.24 |
| Qwen2.5-0.5B (4bit) | 500M | 343.91 | 682.41 | 502.51 |
| Llama-3.2-1B | 1B | 364.36 | 546.81 | 253.63 |
| Qwen3-0.6B | 600M | 275.55 | 566.50 | 317.75* |
| StableLM-1.6B | 1.6B | 270.88 | 425.14 | 197.05 |
| Gemma-3-1B | 1B | 229.70 | 399.65 | 256.48 |
| EXAONE-3.5-2.4B | 2.4B | 197.73 | 282.35 | 146.48 |
| SmolLM3-3B | 3B | 126.29 | 232.79 | 100.66 |
| Nemotron-H-30B | 30B | 91.54 | 177.18 | 32.92 |
| Qwen3-MoE-30B | 30B | 83.75 | 157.16 | 57.49 |
| Llama-3.1-8B | 8B | 107.89 | 116.65 | 49.15 |
| Qwen2.5-7B | 7B | 111.50 | 126.36 | 53.73 |
| Mixtral-8x7B | 47B | 54.25 | 65.20 | 28.00 |
| GPT-OSS-120B | 120B (MoE) | 58.41 | 114.03 | 50.63 |
| Solar-Open-100B | 100B (MoE) | 32.96 | 65.36 | 18.52 |
...
M1 Ultra column is from 2026-06-15 with mlxcel 0.2.1 / MLX pin commit `a6ec712` (0.32.0-dev) / no cooldown, using the `mlxcel-bench-decode` same-process harness (post #289 bf16-scale fix).
...
The two Apple Silicon columns use the same same-process harness but different mlxcel/MLX pins (M1 Ultra on 0.2.1/`a6ec712`, M5 Max on 0.1.0/`84961223`); the M1 Ultra 06-15 run restores bf16-scale decode after the #289 fix, so the gap mostly reflects hardware delta. M5 Max stays roughly 1.7x faster than M1 Ultra on the selected 16 rows (avg ~1.74x, median ~1.86x). The largest MoE rows still show the M5 Max advantage: gpt-oss-120b runs at 114.03 vs 58.41 tok/s (1.95x) and solar-open-100b runs at 65.36 vs 32.96 tok/s (1.98x). The qwen3-30B-A3B MoE row narrows the most after the bf16-scale fix (83.75 vs 70.27 tok/s on 06-12).
...
## Overall Status (mlxcel 0.2.1 on M1 Ultra, 0.1.0 on M5 Max and GB10)

| Metric | Count |
|--------|-------|
| Supported model architectures | 89+ ModelType variants |
| Text models tested (M1 Ultra, 2026-06-15) | 137 ran (135 with nonzero decode), 13 fail, 1 oversize skip (151 dirs; adds apertus, seed-oss, dots.llm1, granite family, lfm2, plamo-2, falcon-h1, BitNet) |
