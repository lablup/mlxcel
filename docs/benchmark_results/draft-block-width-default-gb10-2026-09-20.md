...
**One acceptance criterion is not met.** Greedy output at the new default is not byte-identical to classic decode. Neither is it at any other width, including the 16 that `main` resolves today, so this is the existing behavior rather than something the new default introduces, and nothing gates it on the Qwen 3.5 DFlash path. Filed as #1935.
