...
The host gate earned its place twice. During the mechanism sweep it reported `foreign_models: ['mlxcel']` and held, because another session's model process appeared between arms; during the multirow control described above it reported `busy: ['nvcc']` and held again, because another session started a CUDA compile. Both were gone before the next arm started, and neither landed inside a timed run. That is the predicate working as designed on a shared host, which is the situation the #1820 harness was hardened for, and it is why the gate belongs in front of every arm rather than once at the start of a session.

## What the policy gained, and what it did not

**One entry.** `measured_default_block_size` returns `Some(4)` for compute capability 12.1 with an affine-quantized target on a DFlash drafter, and `None` for everything else. It resolves after an operator override and after a drafter checkpoint's own declared width, and before the flat per-kind constant, so every host and quantization path without a measurement behaves exactly as it did. Both call sites reach it through `resolve_draft_block_size_for_target`, and the worker's startup line names what it matched on: `block_size=4, block_size_from=measured default for sm_121 with an affine-quantized target`.

**No NVFP4 entry.** `(12, 1, Nvfp4)` stays `None` and the test that asserts it stays. The mechanism table above shows widths winning by up to 1.82x, but only with `MLXCEL_MTP_ALLOW_INEXACT=1` set by hand; an unmodified server runs the Laguna pairing's own exactness probe, which declines at every width, so a default there would name a width nothing reads. If that probe is ever satisfied, whether by fixing the verify's numerics or by narrowing what the probe compares, the mechanism table is the measurement an entry would be seeded from, and it points at 7 rather than 4.

**Why a table rather than a constant.** Two families, one host, one binary, one session: the affine pairing plateaus across widths 2 to 4 and loses 14% at 5, the NVFP4 pairing climbs to 7 and loses 12% at 8. Their optima are four widths apart, and each drops exactly where its own kernel says it should, the affine one at `dispatch_multirow_width`'s accumulator boundary and the NVFP4 one at the `M * B < 8` family switch. A single constant cannot be right for both, and the argument for that is now measured rather than reasoned.

**One acceptance criterion is not met.** Greedy output at the new default is not byte-identical to classic decode. Neither is it at any other width, including the 16 that `main` resolves today, so this is the existing behavior rather than something the new default introduces, and nothing gates it on the Qwen 3.5 DFlash path. Filed as #ISSUE.
