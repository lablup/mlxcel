...
## 2026-07-24: numeric-faithfulness fix, f32 partials (#886)

`gemma-4-26b-a4b` produced corrupted tokens in long multi-turn greedy decodes with the fused kernel enabled (stray foreign-script characters such as "During the Middleも Ages", or wrong words) while `MLXCEL_FUSED_MOE=0` stayed clean, on both CUDA and Apple Silicon.

Root cause, established with an in-situ per-call probe (`MLXCEL_FUSED_MOE_PARITY_CHECK=1`, added in #886) over a full 5-turn repro on GB10: the down kernel rounded each per-expert partial `score_k * down_k(act)` to the activation dtype (bf16) BEFORE the host summed the K partials. The K expert outputs routinely near-cancel during decode, so the pre-sum rounding error, which scales with the large partials rather than the small sum, reached 5-13% relative error on the layer's residual update (97 of ~25600 MoE calls exceeded 5% against the gather_qmm reference, worst 12.9%, concentrated on calls whose reference output RMS was smallest). Greedy decode integrates those hits over ~38k MoE calls per run until a marginal token decision lands on a garbage token. The probe also re-ran the fused pair on identical inputs at every call and observed 0 bitwise mismatches in 25600+ calls, and the isolated kernel tests show byte-identical output across the full `MLXCEL_FUSED_MOE_SGY` range, ruling out the suspected reduction/synchronization race: the kernel was always deterministic, just not numerically faithful.

Fix (both the CUDA and Metal twins of `MOE_DOWN`, plus the shared host wrapper and the `MLXCEL_FUSED_MOE_RELU2` fc2 call site): the down kernel now emits f32 partials with the score folded in f32, the host K-sum runs in f32, and the result is rounded to the activation dtype exactly once at the end. Rounding after the cancellation bounds the error at the output's own bf16 quantum (~0.4% relative) regardless of how strongly the experts cancel. Cost: the `[K, Din]` staging buffer doubles to f32 (~90 KB at gemma4 shapes) plus one elementwise cast per call; decode throughput is unchanged within noise.

Verification: `fused_moe_parity_tests` in `mlxcel-core` (run with `MLX_CUDA_ARCHITECTURES=121 cargo test -p mlxcel-core --release --features cuda fused_moe`) pins fused vs all-f32-dense-reference parity at gemma4 decode shapes (nrms < 5e-4 after single-rounding normalization), bitwise rerun determinism under allocator churn, byte-identical output across SGY 1..32, and the kernel-source reduction/rounding structure. The 5-turn 26B repro is clean with the fused kernel enabled after the fix (see #886 for the acceptance runs).
