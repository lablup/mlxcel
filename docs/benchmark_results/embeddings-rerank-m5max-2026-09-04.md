...
When this pass ran, `--media-path` worked but the URL had to be **relative to the media root**. An absolute path was concatenated onto the root rather than replacing it, so `file:///abs/path/tests/fixtures/test_image.png` was probed at `<root>/abs/path/tests/fixtures/test_image.png` and reported as `file does not exist or cannot be opened`. The concatenation itself is deliberate llama-server b10621 compatibility: `resolve_media_file_in` in `src/server/media_root.rs` strips the leading separator before joining, matching upstream's `media_path + file_path`, and `an_absolute_looking_path_is_concatenated_not_joined` in `src/server/media_root_tests.rs` pins it so an absolute URL cannot escape the root.

Verified against a live server with `--media-path <repo>/tests/fixtures`: `file://test_image.png`, `test_image.png` and `file:///test_image.png` all returned HTTP 200 with a 2048-dimension vector, `file://../../etc/passwd` was refused with `file path is not allowed`, and only the absolute form failed. The harness had been sending the absolute form, which is why every image cell returned HTTP 400 on the first run.

Issue #1612 has since made the absolute form resolve. The concatenation stays the primary resolution, and an absolute reference whose concatenated candidate does not resolve is canonicalized and handed to the same containment check, so `file:///abs/path/tests/fixtures/test_image.png` now reads the fixture and an absolute path outside the root is refused as `file path escapes the --media-path root` instead of as a missing file. The `--media-path` `--help` entry and `docs/llama-server-compat.md` state the rule with one accepted absolute and one accepted relative form, and the unresolvable message now names `--media-path` as the directory paths resolve against.

The harness still sends the fixture as a base64 `data:` URI rather than a `file://` URL of either shape. All of them work now; the data URI is kept because it needs no server flag at all, which makes the ladder reproducible on a host where the operator has not set `--media-path`. The fixture is 679 bytes, so inlining it costs nothing, and the resulting visual token counts match the GB10 pass exactly.

One residue is left for a later pass: `_image_data_uri()` in `scripts/bench_embeddings.py` still describes the pre-#1612 behavior in its docstring, and the `--media-path` comment on the server command line still explains itself in terms of the relative form only. Both are correct about why the harness sends a data URI and stale about what a `file://` URL can be, and neither changes a measurement.
