...
One property to know before using these vectors for text-to-text retrieval: SigLIP's text tower is trained against images, never against other texts, so its text-only cosine similarities sit on a high anisotropic floor. Across six deliberately unrelated sentences on `google/siglip-base-patch16-224` the off-diagonal cosines measured 0.52 to 0.73 (mean 0.67), while `a photo of a cat` against `a photo of a kitten` reached 0.966. Compare margins between candidates rather than thresholding the absolute score, which is the opposite of what a sentence-transformers encoder invites.
