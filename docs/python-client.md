...
Generation methods accept OpenAI sampling fields directly: `max_tokens`, `temperature`, `top_p`, `stop`, `seed`, `presence_penalty`, `frequency_penalty`, `logit_bias`, and `response_format`. Server-specific knobs that are not part of the OpenAI schema (`top_k`, `min_p`, `top_n_sigma`, `typical_p`, `repetition_penalty`, DRY settings, `xtc_probability` / `xtc_threshold`, and the vLLM-compatible loop-detection fields `max_pattern_size` / `min_pattern_size` / `min_count`) are forwarded in the request body. `xtc_probability` (`0.0`-`1.0`, default `0.0`, disabled) is the per-step chance that XTC (Exclude Top Choices) removes all but the single least-probable token among those whose probability exceeds `xtc_threshold` (`0.0`-`0.5`, default `0.1`). The threshold is tested against the renormalised distribution that remains after `top_k`, `top_p` and `min_p` have run, not against the raw model distribution, matching the llama-server chain; both are validated on `/v1/chat/completions`, `/v1/completions`, and `/v1/responses`, and an out-of-range value returns a 400 before generation starts. `top_n_sigma` (default `0.0`, disabled) keeps only the tokens whose raw logit lies within N standard deviations of the row maximum before the temperature / top-k / top-p / min-p chain runs, which keeps sampling stable at high temperature; it must be finite and `>= 0.0` on the OpenAI-shaped endpoints (a negative value returns a 400). The native `/completion` spelling of the field lands together with its llama-server compatibility-manifest entry (#1436). `typical_p` (default `1.0`, disabled) enables locally typical sampling: tokens whose surprisal is closest to the row entropy are kept, in typicality order, until `typical_p` probability mass accumulates; it must be finite and in `(0.0, 1.0]` on the OpenAI-shaped endpoints (400 otherwise), the native `/completion` field mirrors llama-server's limit-free schema by resolving a present out-of-range value to the explicit disabled form, and the server-wide `--typical` / `--typical-p` flag sets the default applied when a request omits the field. Since #1436 the repetition, frequency, and presence penalties follow llama-server's `repeat_last_n` window semantics: `repetition_context_size` (alias `repeat_last_n`) bounds them to the last N tokens, `0` disables them, and an absent field falls back to the server-wide `--repeat-last-n` default of 64 (previously the whole history was always scanned). DRY likewise adopts the b10621 sentinels: `dry_penalty_last_n: 0` disables DRY, the CLI's `-1` scans the full history, and a `dry_base` below `1.0` falls back to the server default. Server-wide defaults also exist for `top_n_sigma` (`--top-nsigma`, `-1.0` or `0.0` disabled), XTC (`--xtc-probability`, `--xtc-threshold`), end-of-sequence suppression (`--ignore-eos`), and stop strings (`-r` / `--reverse-prompt`, repeatable). You can also pass an explicit `extra_body={...}` for arbitrary server fields; values you set there win on conflict.
