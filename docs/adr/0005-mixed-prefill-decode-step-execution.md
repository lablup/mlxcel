...
Building (b) therefore means: a packed `[1, N]` layout, per-token rather than per-row RoPE positions, a new segment-loop attention entry point, and logits extraction that picks different rows for the decode segments and the prefill segment. Per family. Llama 3 alone is a prototype; feature parity is every family. And the packed row count `B + C` varies with both the decode batch and the terminal chunk's length, which churns the MLX shape-keyed graph cache in a path that currently sees a small fixed set of shapes.
