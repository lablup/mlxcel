...
**One measurement confound was removed rather than tolerated.** `continue_chunked_prefill` cleared the MLX buffer cache on every chunk. That was harmless while it could only run with an empty batch, but on a mixed tick it would put an unconditional per-step clear on the decode hot path for the whole duration of a long prefill, inflating exactly the inter-token latency the Results table is supposed to read. The decode path deliberately clears on a cadence instead (256 tokens on Metal, off by default on CUDA, because a per-step clear churns the pool and defeats CUDA-graph reuse, ml-explore/mlx#2358). The clear is now suppressed on mixed ticks and left untouched everywhere else, so the prototype arm's ITL reflects scheduling rather than an allocator artifact.
...
**Two behaviour changes the prototype does introduce, neither of them a parity break.** First, paged block *availability*. Before #908 the chunk continuation only ran with an empty active batch, so a decode batch and a chunked prefill never grew pool blocks at the same time. Under `MixedStep` with the default `--kv-cache-budget auto` and paged decode they do, and `admit_paged_prefill` is a point-in-time check rather than a reservation. Exhaustion is graceful (block acquisition returns an error), so the effect is a higher request-failure rate under memory pressure, not a crash. It is compounded slightly by `finalize_completed()` running after the chunk, so sequences the decode half just finished have not yet released their KV when the chunk allocates. Second, the #734 head-of-line bound roughly doubles: `MixedStep` occupies the arbitration slot `Decode` used to, but a mixed tick is two forwards, so a pending speculative slice waits about twice as long between rounds while a chunked prefill is parked. Both belong to the fairness policy the follow-up issue owns.
...
**Cell 2, the ITL price of interleaving, is now measured (issue #1011).** It was left open here because it is the fairness policy's tuning input and belongs to whichever policy ships; that policy shipped as `--prefill-grant-interval N`, which grants the parked prefill one tick after every `N` contended decode ticks. Full table, five intervals times three interleaved repeats: `docs/benchmark_results/prefill-fairness-m1ultra-2026-08-03.md`.

The result is a clean frontier rather than a single number, and it is predictable rather than empirical. One grant cycle is `N` decode ticks plus one chunk, so a `C`-chunk prompt takes `C * (N * D + P)` and the decoding streams' mean inter-token latency over the window is `D + P / N`. On a quiet M1 Ultra, `D` = 58 ms and `P` = 775 ms for a 512-token chunk, and the fit is within 4% at every interval measured. At the shipped default of 16 the admitted request's TTFT falls from 51 s (and unbounded, since the disabled arm's figure is just however long the batch takes to drain) to 23.8 s with a spread of 0.3 s across a 4x range of machine load, and the decoding streams pay 1.6 to 1.8x mean inter-token latency for the duration of the admission window only. Their p50 does not move at all.

Two things worth carrying forward. First, the p95 inflation is 11 to 16x at every interval below 19 and 1.0x at and above it, for the arithmetic reason that exactly one gap in `N + 1` carries a chunk: nothing improves at the larger interval, the same hiccups move from p95 to p99. The lever that shrinks a hiccup rather than relocating it is `--prefill-chunk-size`, which is the same conclusion this ADR reaches about `saving` from the other direction. Second, `P / D` = 13 on this hardware is a per-machine number, so a tick-counted interval grants a different share of wall clock elsewhere; that is the price of a policy that costs one comparison and can be tested without faking a clock.
...
The #1011 quiet-machine run settles that last point: `C/P` is 775 ms per 512-token chunk on an unloaded M1 Ultra, against a 58 ms decode step. `saving` is therefore `D_lin / 833 ms` with `D_lin` under 58 ms, so under 7% of an admission window even before subtracting attention from `D`. That is inside the band this ADR argued from published numbers, at the pessimistic end, and it is the figure a fused ragged forward would be competing for.
...
- **The starvation fix shipped as issue #1011**, and it is the grant counter this section named as the cheaper alternative: `--prefill-grant-interval N` / `MLXCEL_PREFILL_GRANT_INTERVAL`, default 16, `0` to restore the arbitration described above. `TickState` gained the counter and `decide_tick` returns its successor alongside the choice, so the single place that applies a decision cannot take one without the other. Cell 2 above records what the bound costs. Two details of this ADR were superseded rather than confirmed by it: the `mixed_tick` flag that suppressed `continue_chunked_prefill`'s per-chunk buffer-cache clear is now derived from the active batch instead of passed in, because the default policy also runs chunks next to a live decode batch and a caller that forgot the flag would put an allocator clear on the decode hot path; and the paged-block-availability change this section attributes to the prototype now applies by default, for the same reason. `finish_prefill` gained an explicit full-batch guard, because a chunked prefill can now complete against a non-empty batch and `ActiveBatch::add` consumes the sequence it cannot place.
- **The two tests this ADR names by their #908 form were re-aimed, not deleted, by #1011.** `chunked_prefill_starves_until_active_batch_drains` became `a_parked_chunked_prefill_is_never_starved_by_a_live_decode_batch`, with the old fixed point kept as `grant_interval_zero_restores_the_pre_1011_fixed_point` so the escape hatch's promise is pinned. `mixed_step_off_is_identical_to_the_pre_908_policy` lost its premise, since the default policy deliberately changed, and became `default_policy_differs_from_pre_908_only_where_the_grant_fires`: the same exhaustive comparison against the same transcription, now asserting soundness (every divergence is a fired grant) and completeness (every due grant fires) instead of equality. `grant_disabled_is_identical_to_the_pre_908_policy` keeps the original equality proof for `--prefill-grant-interval 0`.
...
- Issue #908, this spike. Epic #909. Issue #1011, the fairness policy that fills cell 2 and fixes the starvation.
- `docs/benchmark_results/prefill-fairness-m1ultra-2026-08-03.md`, the interval sweep behind cell 2.
