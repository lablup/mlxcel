...
Only the library base stylesheet and component subpath styles are imported. `common-tokens.css` maps the full published token contract to product colors, typography, spacing, radii and motion. `common-components.css` scopes necessary DOM/geometry overrides to adapters, including primary/danger foregrounds, compact 44px targets and wrapped controls. No orange theme stylesheet or remote font/resource is loaded. Keep the approved macOS 27 shell topology, continuous toolbar and neutral content surfaces; package adoption is not approval to substitute its default PageLayout or theme.
