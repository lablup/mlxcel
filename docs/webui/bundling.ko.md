...
이슈 #1836은 최소 React 셸, 재현 가능한 정적 자산, 재사용 가능한 Rust 라우터를 제공합니다. 운영용 `--webui` CLI 옵션이나 모델 관리 API는 **아직 활성화하지 않습니다**. 시작 경로 연결은 #1838, 브라우저 인증은 #1837, 운영 페이지는 에픽 #1834의 후속 이슈가 담당합니다. 셸에 표시된 예정 기능을 이미 동작하는 기능으로 해석하지 마세요.
...
브라우저 명령은 Vite preview 서버에서 셸을 확인합니다. 운영 서버·Safari·인증·실모델 인수 테스트가 **아닙니다**. 정적 하네스는 루프백 주소를 출력하고 상태 확인·모델 스텁 경로와 `/webui/`를 제공합니다. 체크포인트를 로드하지 않으며 `mlxcel-server --webui`를 구현하지 않습니다. 로컬 테스트 주소는 `MLXCEL_WEBUI_HARNESS_ADDR`로 지정할 수 있습니다.
