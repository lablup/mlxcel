...
Issue #1836 supplied the minimal React shell, reproducible static assets, and reusable Rust static router. Issue #1838 now mounts that bundle behind `mlxcel-server --webui` and `mlxcel serve --webui` with the WebUI security wrapper and typed UI API; operational pages still arrive through dependent issues of epic #1834. The shell labels capabilities by their implemented backend status rather than implying that later chat, visualization, download, or rich metrics pages are complete.
...
The browser command starts a Vite preview server and checks the scaffold. It is **not** a production-server, Safari, authentication, or real-model acceptance test. The static harness prints its loopback address and serves `/webui/` alongside stub health/model routes; it remains useful for relocation and embedding checks, but it is not evidence that `mlxcel-server --webui` startup, generated terminal credentials, Host/Origin/Fetch-Metadata checks, or model-free catalog access work. Validate those through the production binaries and mounted routes.
