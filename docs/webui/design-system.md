...
The component gallery is available at `#gallery`. `pnpm --dir webui run unit` currently executes 10 Vitest tests for shell shortcuts, storage, typed strings, LoginView and SchemaMismatchView. `pnpm --dir webui run browser` currently executes 12 Playwright tests: 8 committed screenshot baselines per platform, plus production-route honesty, keyboard/focus/shortcut behavior, contrast/reduced-motion/visibility behavior and backdrop-filter fallback coverage. The screenshot baselines are `1440-light-gallery-controls.png`, `1440-light-gallery-data.png`, `1440-dark-gallery-controls.png`, `1440-dark-gallery-data.png`, `1024-tinted-gallery-data-inspector.png`, `390-opaque-gallery-controls-drawer-cjk.png`, `1440-highcontrast-gallery-states.png` and `390-dark-opaque-textscale200-gallery-controls.png`; each uses axe with all violations rejected and Playwright screenshot comparison with `maxDiffPixelRatio: 0.005`. The text-scale case uses a test-only root attribute to exercise reflow with 200% text metrics in Gallery controls and tabs, including document/key-panel horizontal overflow assertions and reachability checks for full tab, button and field labels; it is not claimed as native browser zoom. These screenshots are implementation baselines for downstream page work and still require human screenshot approval before page-specific styling proceeds.
