...
Metadata projection runs on blocking workers. A bounded cache avoids repeating recursive disk accounting, config parsing, and content-fingerprint work on ordinary polling cache hits; lifecycle, revisions, provider-confirmed capabilities, and removal eligibility are still refreshed from current pool state. The first cache fill and explicit refresh perform bounded filesystem inspection. Explicit refresh clears cached metadata and performs the existing router rescan; legacy router reloads also advance the catalog epoch so stale cached metadata is not permanent.
