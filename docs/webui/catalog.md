...
`metadata.dtype` is the declared weight dtype from `config.json`: `torch_dtype`, or `dtype` in newer exports, read at the top level and then under `text_config`. It is normalized to a short lowercase name (`bfloat16` becomes `bf16`, `float16` becomes `fp16`, `float32` becomes `fp32`, and a `torch.` prefix is dropped); any other value is reported lowercased only when it is an ASCII identifier of at most 32 bytes. It is `null`, with no separate reason, when the key is absent, not a string, `auto`, or outside that bound. Quantized checkpoints also report it, naming the dtype of their unquantized tensors, so the library shows `metadata.quantization` when it is known and falls back to `metadata.dtype` for unquantized checkpoints.
