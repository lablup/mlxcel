...
SSE events are monotonic per server instance. Event IDs are opaque strings containing sequence and instance information; clients compare only by server-provided replay behavior. Unknown IDs, a ring miss, a changed instance, or a reset envelope force `/bootstrap`, `/catalog`, `/operations`, and selected `/runtime` refetch. Reconnect never re-POSTs actions.
