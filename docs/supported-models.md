...
- DeepSeek v1 / v2 / v3 / v3.2 / v4 (backbone)
- DeepSeek v4 backbone (`deepseek_v4`, the next-generation DeepSeek MoE. mlxcel ports the backbone only: it reuses the DeepSeek-V3 Multi-head Latent Attention, group-limited sigmoid-routed MoE, and unclamped shared-expert `DenseMLP`, plus the DeepSeek-V3 multi-token-prediction (MTP) layer handling, by mapping the `deepseek_v4` config onto the V3 backbone. The two V4-specific features, HiSA hierarchical sparse attention and the shared-expert SwiGLU `swiglu_limit` clamp, are tracked as separate follow-up issues and are parsed but not yet applied. Config port and V3-backbone parity are covered by `tests/deepseek_v4_parity.rs`; real-checkpoint validation is pending a public `deepseek_v4` export.)
