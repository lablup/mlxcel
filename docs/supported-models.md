...
- MiniMax-M3 (`minimax_m3`, MiniMax: a hybrid dense/MoE decoder that follows MiniMax-M2 (`minimax`) but swaps in Gemma-style RMSNorm (weight+1) everywhere including per-head Q/K norm, clamp-SwiGLU (`swigluoai`) experts and dense MLPs, partial RoPE (`rotary_dim` of `head_dim`), and a sigmoid router with a selection-only routing bias plus one shared expert. The per-layer plan comes from `moe_layer_freq` (leading dense layers, then MoE); the shared expert is packed as switch-tensor index `num_local_experts` with a fixed score of 1.0 when its width equals the routed width, otherwise a separate MLP. The sparse layers (`sparse_attention_config.sparse_attention_freq`) add a block-sparse "MSA" indexer: per-head index Q/K projections with Gemma norm and RoPE, top-`sparse_topk_blocks` selection over `sparse_block_size` key blocks scored by their max token score, expanded to an additive block mask on top of the causal mask, with the index key cached alongside the regular K. When the selected window still covers at least half the live cache, attention stays dense; when `sparse_topk_blocks` covers every block, the block mask is all zeros and attention is bit-identical to dense. Only the text decoder is implemented; the VL wrapper and MTP head are out of scope (MTP tensors are skipped on load). The only public checkpoint is the 427B `MiniMaxAI/MiniMax-M3` VL repo, which exceeds the development machine's memory, so validation is synthetic reduced-config unit tests plus a real-`text_config` parse test; real-checkpoint load/generate carries over to a follow-up runtime validation.)
- InternLM 2 / 3
